...
const CATEGORY_TO_RESPONSIBILITY: Record<
  ReviewIssueCategory,
  ResponsibilityCode[]
> = {
...
export function normalizeRating(
  ...
  const safeScale =
    Number.isFinite(numericScale) && numericScale > 0 ? numericScale : 5;
...
function detectCategories(text: string): GuestReviewIssue[] {
  ...
  >) {
    ...
    detected.push({
      ...
      severity: ["safety_security", "maintenance"].includes(category)
        ? "high"
        : "medium",
      confidence: 0.78,
      polarity: "negative",
...
export function buildFallbackAnalysis(
  ...
  const positiveMentions = POSITIVE_WORDS.filter((word) =>
    lower.includes(word),
  );
...
export async function callStructuredAi(
  ...
  try {
    ...
    if (!response.ok || !payload?.success) {
      throw new Error(
        payload?.error || `AI request failed with HTTP ${response.status}`,
      );
    }
    ...
    console.error(
      `${logPrefix} AI request failed after ${maxRetries} retries:`,
      error,
    );
    throw error;
  }
...
function coerceSentiment(
  value: unknown,
): GuestReviewAnalysisResult["sentiment"] {
...
function normalizeAnalysisPayload(
    ...
    ? (aiPayload.issues
        .map((issue) => {
          ...
          return {
            ...
            summary_en: String(
              issueRecord.summaryEn ||
                issueRecord.summary_en ||
                fallback.summaryEn,
            ),
            summary_ar: String(
              issueRecord.summaryAr ||
                issueRecord.summary_ar ||
                fallback.summaryAr,
            ),
            evidence_text: String(
              issueRecord.evidenceText || issueRecord.evidence_text || "",
            ),
  ...
  return {
    ...
    draftResponseEn: String(
      aiPayload.draftResponseEn || fallback.draftResponseEn,
    ),
    draftResponseAr: String(
      aiPayload.draftResponseAr || fallback.draftResponseAr,
    ),
...
export function formatRatingDisplay(
  review: Pick<
    GuestReviewRecord,
    "rating_normalized_10" | "rating_normalized_5"
  >,
): string {
...
async function resolveFallbackAssignee(
  ...
  if (responsibilityCode === "general_manager") {
    const { data } = await supabase
      .from("profiles")
      .select(
        "id, full_name, email, user_roles!inner(role), user_properties!inner(property_id)",
      )
...
export async function resolveAssignmentOwner(
  ...
  const primaryProfile = mappings?.primary_profile as
    | Record<string, unknown>
    | null
    | undefined;
  const backupProfile = mappings?.backup_profile as
    | Record<string, unknown>
    | null
    | undefined;
  ...
  if (primaryProfile?.id || backupProfile?.id) {
    return {
      assigneeProfileId:
        typeof primaryProfile?.id === "string" ? primaryProfile.id : null,
      backupProfileId:
        typeof backupProfile?.id === "string" ? backupProfile.id : null,
      assigneeName: String(
        primaryProfile?.full_name || responsibilityCode.replace(/_/g, " "),
      ),
      assigneeEmail:
        typeof primaryProfile?.email === "string" ? primaryProfile.email : null,
      backupEmail:
        typeof backupProfile?.email === "string" ? backupProfile.email : null,
    };
  }
...
function collectResponsibilities(
  issues: GuestReviewIssue[],
): Map<ResponsibilityCode, GuestReviewIssue[]> {
  const map = new Map<ResponsibilityCode, GuestReviewIssue[]>();

  for (const issue of issues) {
    const responsibilities = CATEGORY_TO_RESPONSIBILITY[issue.category] ?? [
      "general_manager",
    ];
...
export async function enqueueGuestReviewNotifications(
  ...
  const issueCategories = Array.from(
    new Set(analysis.issues.map((issue) => issue.category)),
  ).join(", ");
  ...
  for (const assignment of assignments) {
    ...
    if (assigneeEmail) {
      queueRows.push({
        ...
        notification_kind: analysis.critical
          ? "critical_alert"
          : "review_alert",
        channel: "email",
        payload,
    ...
    for (const endpoint of endpointRows) {
      const endpointPropertyId =
        typeof endpoint.property_id === "string" ? endpoint.property_id : null;
      const endpointScope = String(endpoint.scope || "property");
      const endpointResponsibility =
        typeof endpoint.responsibility_code === "string"
          ? endpoint.responsibility_code
          : null;

      const propertyMatch =
        endpointPropertyId === null ||
        endpointPropertyId === review.property_id;
      const responsibilityMatch =
        endpointResponsibility === null ||
        endpointResponsibility === responsibilityCode;
      const scopeMatch =
      ...
      queueRows.push({
        ...
        payload: {
          ...
          recipientEmails: Array.isArray(endpoint.recipients)
            ? endpoint.recipients
...
export async function runGuestReviewAnalysis(
  ...
  const { data: reviewData, error: reviewError } = await supabase
    .from("guest_reviews")
    .select(
      "id, property_id, platform, review_url, reviewer_name, review_title, review_text, review_language, rating_normalized_10, rating_normalized_5, vip_flag, source_id, summary_en",
    )
    .eq("id", reviewId)
    .single();
  ...
  const tone = (settingResult.data?.default_tone || "business") as
    | "luxury"
    | "business"
    | "casual_hospitality";
  ...
  try {
    ...
    console.warn(
      `${logPrefix} guest-review-analysis fallback triggered:`,
  ...
  if (
    (analysis.sentiment === "negative" ||
      (review.rating_normalized_10 ?? 10) <= 6) &&
    !responsibilities.has("general_manager")
  ) {
    responsibilities.set("general_manager", []);
  }
  ...
  for (const [
  ...
  ] of responsibilities.entries()) {
    const fallbackResponsibility =
      responsibilityCode === "it_manager"
        ? "maintenance_manager"
        : responsibilityCode;
    const owner = await resolveAssignmentOwner(
      supabase,
      review.property_id,
      responsibilityCode,
    );
    const resolvedOwner =
      owner.assigneeProfileId || owner.backupProfileId
        ? owner
        : fallbackResponsibility !== responsibilityCode
          ? await resolveAssignmentOwner(
              supabase,
              review.property_id,
    ...
    const categories = Array.from(
      new Set(responsibilityIssues.map((issue) => issue.category)),
    );
    const issueIds = categories.flatMap(
      (category) => issueIdByCategory.get(category) ?? [],
    );
    const isSecondary =
      responsibilityCode === "general_manager" &&
      responsibilityIssues.length === 0;
    const routingReason = isSecondary
      ? "negative_gm_copy"
      : "ai_category_detection";

    assignmentRows.push({
      review_id: review.id,
      property_id: review.property_id,
      responsibility_code:
        resolvedOwner.assigneeProfileId || resolvedOwner.backupProfileId
          ? responsibilityCode
          : fallbackResponsibility,
  ...
  const { data: insertedAssignments, error: assignmentInsertError } =
    await supabase
      .from("guest_review_assignments")
      .insert(
        assignmentRows.map(({ assignee_name, assignee_email, ...row }) => row),
      )
  ...
  for (const row of (insertedAssignments ?? []) as Array<
    Record<string, unknown>
  >) {
  ...
  await enqueueGuestReviewNotifications(
    supabase,
    review,
    propertyName,
    analysis,
