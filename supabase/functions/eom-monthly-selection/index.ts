...
Deno.serve(async (req: Request) => {
    ...
    try {
        ...
        const isServiceRole = isAuthorizedServiceRoleRequest(authHeader, serviceRoleKey)
        const isAdminRequest = req.headers.get('X-Admin-Request') === 'true' && isServiceRole
        ...
        console.log(`EOM Monthly Selection Job - ${currentYear}-${String(currentMonth).padStart(2, '0')}-${String(currentDay).padStart(2, '0')}`)
        ...
        if (configError) {
            throw new Error(`Failed to fetch automation configs: ${configError.message}`)
        }
        ...
        for (const config of configs) {
            ...
            if (!targetPropertyId && !dryRun && currentDay !== announcementDay) {
                console.log(`Skipping ${propertyName} - announcement day is ${announcementDay}, today is ${currentDay}`)
                continue
            }
            ...
            try {
                ...
                const selectionYear = currentMonth === 1 ? currentYear - 1 : currentYear

                // Run the calculation
                const { data: calculationResults, error: calcError } = await supabaseClient
                ...
                console.log(`Top 3 for ${propertyName}:`, top3.map((r: any) => `${r.full_name} (${r.total_score})`).join(', '))
                ...
                const { data: selectionId, error: selectionError } = await supabaseClient
                    .rpc('generate_eom_auto_selection', {
                        p_property_id: propertyId,
                        p_month: selectionMonth,
                        p_year: selectionYear
                    })
                ...
                if (selectionError) {
                    throw new Error(`Selection generation failed: ${selectionError.message}`)
                }
                ...
                if (selection.status === 'announced' && selection.user_id) {
                    const monthName = new Date(selectionYear, selectionMonth - 1)
                ...
                console.log(`✅ ${propertyName}: Winner ${selection.profile?.full_name} (${selection.status})`)
        ...
        // RETURN RESULTS
        // ===================================
        const successCount = results.filter(r => r.status === 'success' || r.status === 'dry_run').length
