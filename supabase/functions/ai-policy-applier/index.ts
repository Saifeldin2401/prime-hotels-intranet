...
const parsePath = (path: string) =>
  path.split('.').filter((segment) => segment.length > 0).map((segment) => {
    const asNumber = Number(segment)
    return Number.isInteger(asNumber) && segment === asNumber.toString() ? asNumber : segment
  })
...
const applyPatch = (obj: Record<string, unknown>, change: Record<string, unknown>) => {
...
const applyProposal = async (supabase: any, proposal: any) => {
  if (!['validated'].includes(proposal.status)) {
    throw new Error(`Proposal status ${proposal.status} is not eligible for apply`)
  }
  ...
  if (proposal.policy_set_id) {
    ...
    policySetQuery = policySetQuery.eq('name', proposal.proposal_json.policy_set)
  } else {
    throw new Error('Proposal missing policy_set_id and policy_set name')
  }

  const { data: policySet, error: policySetError } = await policySetQuery.single()
  ...
  const updatedPolicy = JSON.parse(JSON.stringify(activeVersion.policy_json || {}))
  ...
  if (insertError || !newVersion) {
    throw new Error(insertError?.message || 'Failed to create new policy version')
  }
  ...
  await supabase.from('ai_policy_changes').insert({
    ...
    risk_score: proposal.risk_score ?? proposal.proposal_json?.risk_score ?? null,
  ...
  return { proposal_id: proposal.id, new_version: newVersionLabel, policy_set_id: policySet.id }
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (apply_all) {
      ...
      const autoApplyMax = typeof thresholds?.auto_apply_max === 'number'
