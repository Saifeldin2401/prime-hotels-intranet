...
Deno.serve(async (req: Request) => {
  ...
  const emailRes = await fetch(
    `${base}/emails/receiving/${encodeURIComponent(emailId)}`,
    {
      method: "GET",
  ...
  if (!emailRes.ok) {
    ...
    await serviceClient
      .from("inbound_emails")
      .update({
        content_fetch_error: `${emailRes.status} ${text}`.slice(0, 500),
      })
      .eq("email_id", emailId);
  ...
  const attRes = await fetch(
    `${base}/emails/receiving/${encodeURIComponent(emailId)}/attachments`,
    {
      method: "GET",
      headers: buildResendHeaders(RESEND_API_KEY),
    },
  ...
  if (updateError) {
    return jsonResponse(
      { error: "Failed to store received email content" },
      500,
    );
  }
