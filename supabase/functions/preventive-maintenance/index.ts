  ...
  try {
    ...
    for (const schedule of validSchedules) {
      try {
        ...
        const { error: ticketError } = await supabase
          ...
          .insert({
            ...
            description:
              ...
              `Automated maintenance task from schedule: ${schedule.title}`,
        ...
        if (ticketError) {
          console.error(
            `Failed to create ticket for schedule ${schedule.id}:`,
            ticketError,
          );
        ...
        if (updateError) {
          ...
          processingResults.push({
            ...
            message: "Ticket created but schedule update failed",
          });
        } else {
