...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "metrics_only" || action === "full_cycle") {
      results.metrics = await callFunction(
        baseUrl,
    ...
    if (action === "validate_pending") {
      results.validation = await callFunction(
        baseUrl,
        "ai-safety-validator",
        serviceRoleToken,
        {},
    ...
    const validationStatus = (results.validation as any)?.data?.decision
      ?.status;
    if (action === "apply_pending") {
      results.applier = await callFunction(
        baseUrl,
        "ai-policy-applier",
        serviceRoleToken,
        {
    ...
    if (action === "rollback_check" || action === "full_cycle") {
      results.rollback = await callFunction(
        baseUrl,
        "ai-rollback-engine",
        serviceRoleToken,
      );
