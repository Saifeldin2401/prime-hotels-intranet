...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const moduleIds = Array.from(
      new Set(
        (upcomingAssignments || []).map((a) => a.content_id).filter(Boolean),
      ),
    );
    ...
    for (const assignment of upcomingAssignments || []) {
      ...
      if (targetIds.length > 0) {
        ...
        if (progressError) {
          console.error(
            "Failed to check completion for assignment:",
            assignment.id,
            progressError,
          );
        } else {
      ...
      for (const target of targets) {
        ...
        const reminderMatch = resolveReminderWindow(
          assignment.due_date,
          now,
          reminderConfig,
        );
    ...
    if (emailJobs.length > 0) {
      console.log(
        `Triggering ${emailJobs.length} learning emails with rate limiting...`,
      );
    ...
    if (notifications.length > 0) {
      ...
      if (notifError)
        console.error("Error sending detailed notifications:", notifError);
    }
    ...
    if (reminderRows.length > 0) {
      ...
      if (reminderError)
        console.error("Error storing reminder tracking rows:", reminderError);
    }
...
async function dispatchEmailJobs(
  jobs: Array<{ to: string; body: Record<string, unknown> }>,
): Promise<void> {
