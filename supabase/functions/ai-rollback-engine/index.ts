...
serve(async (req) => {
  ...
  try {
    ...
    const slaBreachThreshold = safeNumber(rollbackConditions.sla_breach_rate_gt, 0.05)
    const cycleIncreaseThreshold = safeNumber(rollbackConditions.cycle_time_increase_gt, 0.1)
    ...
    if (!pendingChanges || pendingChanges.length === 0) {
      return new Response(JSON.stringify({ success: true, message: 'No changes to evaluate' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    for (const change of pendingChanges) {
      if (!change.applied_at || !change.from_version_id || !change.to_version_id) {
        continue
      }
      ...
      const baselineCycle = safeNumber(baselineMetrics.requests_avg_cycle_hours, 0)
      ...
      const cycleDegraded = baselineCycle > 0
