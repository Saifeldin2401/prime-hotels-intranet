...
serve(async (req) => {
  ...
  try {
    ...
    const model = body.model || Deno.env.get('AI_OPTIMIZER_MODEL') || defaultModel
    ...
    const userPrompt = JSON.stringify({
      schema: {
        ...
        changes: [
          { action: 'adjust_escalation_hours', path: 'escalation_rules.0.after_hours', value: 36, old: 48 },
        ],
        risk_score: 0.12,
    ...
    if (aiData.error) {
      if (typeof aiData.error === 'string' && aiData.error.includes('loading')) {
        throw new Error('Hugging Face model is loading. Retry in 20-30 seconds.')
      }
      throw new Error(aiData.error.message || aiData.error || 'Hugging Face request failed')
    }
    ...
    const riskScore = typeof proposalJson.risk_score === 'number'
      ? proposalJson.risk_score
      : 0
