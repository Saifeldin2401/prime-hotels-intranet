...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: workflowPolicyRows, error: workflowPolicyError } =
      await supabaseClient.rpc("get_active_policy", { p_domain: "workflow" });

    if (workflowPolicyError) {
      console.warn(
        "Failed to load workflow policy:",
        workflowPolicyError.message,
      );
    }

    const workflowPolicy = workflowPolicyRows?.[0]?.policy_json ?? null;

    const { data: taskPolicyRows, error: taskPolicyError } =
      await supabaseClient.rpc("get_active_policy", { p_domain: "task" });
    ...
    for (const workflow of workflows || []) {
      ...
      if (triggerConfig.cron && triggerConfig.cron.includes("* * *")) {
        ...
        if (execError) {
          console.error(
            `Failed to create execution for ${workflow.name}:`,
            execError,
          );
          continue;
        }
...
async function executeWorkflowAction(
  ...
  switch (action_config.action) {
    case "send_training_reminders":
      return await sendTrainingReminders(
        supabaseClient,
        action_config,
        workflowPolicy,
      );
    ...
    case "notify_overdue_tasks":
      return await notifyOverdueTasks(
        supabaseClient,
        action_config,
        taskPolicy,
      );
...
async function sendTrainingReminders(
  ...
  const policyDays =
    policy?.scheduled_workflows?.training_reminders?.days_before;
  ...
  for (const daysBefore of daysBeforeArray) {
    ...
    const moduleIds = Array.from(
      new Set(
        (assignments || []).map((a: any) => a.content_id).filter(Boolean),
      ),
    );
    ...
    if (moduleIds.length > 0) {
      ...
      if (modulesError) {
        console.warn(
          "Failed to load training module titles:",
    ...
    for (const assignment of assignments) {
      const userIds = await resolveAssignmentTargets(
        supabaseClient,
        assignment.target_type,
        assignment.target_id,
      );
      const moduleTitle =
        moduleTitleById.get(assignment.content_id) || "Training Module";
