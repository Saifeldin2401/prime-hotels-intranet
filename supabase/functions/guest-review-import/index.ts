...
const ALLOWED_ROLES = new Set(["corporate_admin", "regional_admin", "regional_hr", "property_manager", "property_hr"]);
const ALLOWED_PLATFORMS = new Set(["google", "booking", "expedia", "tripadvisor", "agoda", "hotels_com", "airbnb", "manual_import"]);
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const platform = typeof body.platform === "string" && ALLOWED_PLATFORMS.has(body.platform) ? body.platform : "manual_import";
    const rows = Array.isArray(body.rows) ? body.rows : Array.isArray(body.reviews) ? body.reviews : [];
    if (!propertyId || rows.length === 0) {
      return new Response(JSON.stringify({ error: "property_id and rows/reviews are required" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const isServiceRole = authHeader === `Bearer ${serviceRoleKey}`
      || (typeof vaultServiceSecret?.decrypted_secret === "string" && authHeader === `Bearer ${vaultServiceSecret.decrypted_secret}`);
    ...
    if (!isServiceRole) {
      ...
      const canAccessAll = roles.some((r) => r === "corporate_admin" || r === "regional_admin" || r === "regional_hr");
      const allowedPropertyIds = new Set((propRows ?? []).map((p) => String(p.property_id)));
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        const reviewText = cleanText(row.review_text ?? row.text ?? row.content);
        if (!reviewText) continue;

        const reviewerName = typeof row.reviewer_name === "string" ? row.reviewer_name.trim() : null;
        const sourceReviewId = typeof row.source_review_id === "string" ? row.source_review_id.trim() : null;
        ...
        if (!existing) {
          const { data } = await serviceClient.from("guest_reviews").select("id").eq("dedupe_hash", dedupeHash).maybeSingle();
          if (data?.id) existing = { id: String(data.id) };
        }
        ...
        const payload = {
          ...
          review_url: typeof row.review_url === "string" ? row.review_url : null,
          reviewer_name: reviewerName,
          review_title: typeof row.review_title === "string" ? row.review_title : null,
          review_text: reviewText,
          review_text_normalized: reviewText.toLowerCase(),
          review_language: typeof row.review_language === "string" ? row.review_language : null,
        ...
        let reviewId: string;
        if (existing) {
          const { error } = await serviceClient.from("guest_reviews").update(payload).eq("id", existing.id);
          if (error) throw error;
          reviewId = existing.id;
          updated.push(reviewId);
        } else {
          const { data, error } = await serviceClient.from("guest_reviews").insert(payload).select("id").single();
        ...
        await serviceClient.from("guest_review_raw_snapshots").insert({
          review_id: reviewId,
          source_url: typeof row.review_url === "string" ? row.review_url : null,
