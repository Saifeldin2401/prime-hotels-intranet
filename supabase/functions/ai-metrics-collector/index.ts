...
serve(async (req) => {
  ...
  try {
    ...
    const windowHours = typeof body.window_hours === 'number' ? body.window_hours : 24
    ...
    if (closedRequests && closedRequests.length > 0) {
      const totalMs = closedRequests.reduce((sum, req) => {
        ...
        return sum + (new Date(req.closed_at).getTime() - new Date(req.submitted_at).getTime())
      }, 0)
    ...
    const slaBreachRate = openTasks && openTasks > 0
      ? (overdueTasks ?? 0) / openTasks
      : 0
