...
const RSS_FEEDS = [
  // Working Feed
  { url: 'https://www.hoteliermiddleeast.com/feed', source: 'Hotelier Middle East', lang: 'en' },
...
const KEYWORDS = ['Saudi', 'KSA', 'Riyadh', 'Jeddah', 'NEOM', 'Red Sea', 'Hospitality', 'Hotel', 'Tourism', 'Travel', 'Vision 2030'];
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const feed of RSS_FEEDS) {
      ...
      try {
        ...
        for (const item of items) {
          ...
          const hasKeyword = KEYWORDS.some(keyword => contentToScan.toLowerCase().includes(keyword.toLowerCase()));
          ...
          const { error } = await supabase.from('hospitality_news').insert(article);
          if (error) {
    ...
    return new Response(JSON.stringify({ message: 'Success', new_articles: totalNewArticles }), {
      headers: { "Content-Type": "application/json" },
    });
