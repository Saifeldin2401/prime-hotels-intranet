...
export default async (req: Request) => {
  ...
  try {
    const { documentId, content, title, description, fileExtension } = await req.json()
...
async function generateDocumentTags(
  ...
  const tagCategories: Record<string, string[]> = {
    'Operations': ['operations', 'housekeeping', 'maintenance', 'front desk', 'reception', 'check-in', 'check-out', 'room service', 'laundry'],
    'Guest Services': ['guest', 'customer', 'service', 'hospitality', 'concierge', 'amenities', 'complaint', 'review', 'feedback'],
    'HR & Training': ['hr', 'training', 'employee', 'staff', 'onboarding', 'policy', 'procedure', 'manual', 'handbook'],
    'Finance': ['finance', 'accounting', 'billing', 'invoice', 'budget', 'payment', 'revenue', 'cost'],
    'Safety & Compliance': ['safety', 'security', 'compliance', 'regulation', 'emergency', 'fire', 'health', 'sop'],
    'Food & Beverage': ['food', 'beverage', 'restaurant', 'menu', 'kitchen', 'cooking', 'dining', 'bar'],
    'Sales & Marketing': ['sales', 'marketing', 'promotion', 'booking', 'reservation', 'rate', 'pricing', 'advertising'],
    'Technology': ['technology', 'system', 'software', 'app', 'digital', 'it', 'computer', 'network'],
    'Management': ['management', 'manager', 'leadership', 'supervisor', 'director', 'executive', 'gm'],
    'Events': ['event', 'conference', 'banquet', 'meeting', 'wedding', 'party', 'group']
  }
...
function generateSummary(title: string, description: string, content: string): string {
