...
function buildSlackBlocks(payload: Record<string, unknown>) {
  ...
  const reviewedAt = payload.reviewedAt
    ? new Date(String(payload.reviewedAt)).toLocaleDateString("en-US", {
        year: "numeric",
        month: "short",
        day: "numeric",
        hour: "2-digit",
  ...
  const blocks: Array<Record<string, unknown>> = [
    {
      ...
      text: {
        type: "plain_text",
        text: `${severityEmoji} ${propertyName} | Guest Review Alert`,
        emoji: true,
      },
    },
    {
  ...
  // Add Workflow Buttons handled by slack-interactive
  const reviewId = payload.reviewId || payload.id;
  if (reviewId) {
    buttons.push({
      type: "button",
      text: { type: "plain_text", text: "Acknowledge", emoji: true },
      action_id: `review_ack_${reviewId}`,
    });
    ...
    buttons.push({
      ...
      style: "danger",
      action_id: `review_escalate_${reviewId}`,
    });
  }
  ...
  if (reviewUrl) {
    buttons.push({
      type: "button",
      text: { type: "plain_text", text: "Original", emoji: true },
      url: reviewUrl,
    });
  }
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const vaultServiceRoleKey = await getVaultSecret(
      serviceClient,
      "service_role_key",
    );
    ...
    const isInternalService =
      isServiceRole || isVaultServiceRole || isServiceRoleJwt(authHeader);
    ...
    if (isTestMode) {
      const secretName =
        typeof body.secret_name === "string" && body.secret_name.trim()
      ...
      if (!webhookUrl) {
        return new Response(
          JSON.stringify({
            success: false,
            error: `Vault secret "${secretName}" is not configured`,
          }),
          {
            status: 400,
            headers: corsHeaders,
        ...
        blocks: [
          {
            ...
            text: {
              type: "mrkdwn",
              text: "*PHG Connect*: Guest review notifier is reachable.",
            },
          },
        ],
      });
    ...
    for (const row of queueRows ?? []) {
      ...
      try {
        if (row.channel === "slack") {
          const secretName =
            typeof payload.secretName === "string"
              ? payload.secretName
              : "SLACK_GUEST_REVIEWS_WEBHOOK";
          const webhookUrl = await getVaultSecret(serviceClient, secretName);
          if (!webhookUrl)
            throw new Error(
              `Slack webhook secret "${secretName}" was not found`,
            );
          await sendSlackWebhook(webhookUrl, {
          ...
          const res = await fetch(`${supabaseUrl}/functions/v1/send-email`, {
            ...
            body: JSON.stringify({
              ...
              businessDomain:
                row.notification_kind === "daily_exec_digest"
                  ? "management"
                  : "operations",
              notificationType:
                row.notification_kind === "escalation_alert"
                  ? "escalation_alert"
                  : "system",
              title: `${payload.propertyName || "PHG Property"} guest review`,
              message:
                payload.managerBriefEn ||
                payload.summaryEn ||
                "A guest review requires attention.",
              ...
              variables: {
                ...
                issue_categories:
                  payload.issueCategories || "General review follow-up",
          ...
          if (!res.ok || !sendResult?.success) {
            throw new Error(
              sendResult?.error || `send-email failed HTTP ${res.status}`,
            );
          }
        } else {
        ...
        const failImmediately =
          row.channel === "email" &&
          message === "Email notification missing recipients";
        await serviceClient
          .from("guest_review_notification_queue")
          .update({
            status:
              failImmediately || nextAttempt >= Number(row.max_attempts || 5)
                ? "failed"
                : "pending",
            last_error: message,
          })
