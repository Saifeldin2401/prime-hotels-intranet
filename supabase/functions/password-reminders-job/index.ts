...
Deno.serve(async (req: Request) => {
  ...
  const isServiceRoleCall = (authHeader?.replace(/^Bearer\s+/i, "").trim() === serviceRoleKey.trim()) ||
      (req.headers.get("apikey")?.trim() === serviceRoleKey.trim());
  ...
  try {
      ...
      if (!users || users.length === 0) {
          return new Response(JSON.stringify({ message: "No users need reminders", count: 0 }), { 
              status: 200,
              headers: { "Content-Type": "application/json" }
          });
      }

      console.log(`Found ${users.length} users with temp passwords. Fetching template...`);
