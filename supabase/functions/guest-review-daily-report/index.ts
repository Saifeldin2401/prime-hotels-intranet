...
function calculateTrend(
  ...
  if (previous === 0)
    return { direction: current > 0 ? "up" : "stable", change: current };
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: reviews, error: reviewsErr } = await supabase
      .from("guest_reviews")
      .select(
        "id,property_id,platform,sentiment,severity,status,rating_normalized_10,summary_en,review_text,review_title,reviewer_name,reviewer_location,critical_flag,response_sla_due_at,responded_at,created_at,collected_at,published_at,first_acknowledged_at",
      )
      .or(
    ...
    const reviewRows = filterReviewsInWindow(reviews ?? [], window).sort(
    ...
    if (totalReviews === 0) {
      return new Response(
        JSON.stringify({
          success: true,
          message: "No reviews found for this window.",
        }),
        {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const criticalReviews = reviewRows.filter(
      (r) => r.rating_normalized_10 != null && r.rating_normalized_10 <= 5,
    );
    ...
    const { data: prevReviews } = await supabase
      .from("guest_reviews")
      .select(
        "id,rating_normalized_10,sentiment,published_at,collected_at,created_at",
      )
      .or(
    ...
    const prevReviewRows = filterReviewsInWindow(
      prevReviews ?? [],
      previousWindow,
    );
    ...
    (issues ?? []).forEach((iss) => {
      ...
      if (
        ...
        severityDistribution[
          iss.severity as keyof typeof severityDistribution
        ] !== undefined
      ) {
        severityDistribution[
          iss.severity as keyof typeof severityDistribution
    ...
    const averageRating =
      rated.length > 0
        ...
        : 0;
    const prevRated = prevReviewRows.filter(
    ...
    const negativeReviewsCount = reviewRows.filter(
      (r: any) =>
        r.sentiment === "negative" || Number(r.rating_normalized_10 ?? 10) <= 6,
    ).length;
    const prevNegativeCount = prevReviewRows.filter(
      (r: any) =>
        r.sentiment === "negative" || Number(r.rating_normalized_10 ?? 10) <= 6,
    ).length;

    const slaMet = reviewRows.filter(
      (r) =>
        !r.response_sla_due_at ||
        (r.responded_at &&
          new Date(r.responded_at) <= new Date(r.response_sla_due_at)),
    ).length;
    const slaCompliance =
      totalReviews > 0
        ? Number(((slaMet / totalReviews) * 100).toFixed(1))
        : 100;
    ...
    for (const r of reviewRows) {
      ...
      const entry = platformStats.get(key) ?? {
        count: 0,
        avgRating: 0,
        ratedCount: 0,
        totalRating: 0,
    ...
    const platformBreakdown = Array.from(platformStats.entries())
      .map(([platform, stats]) => ({
        ...
        avgRating:
          stats.ratedCount > 0
            ? Number((stats.totalRating / stats.ratedCount).toFixed(1))
            : 0,
      }))
    ...
    for (const r of reviewRows) {
      ...
      if (r.first_acknowledged_at && r.collected_at) {
        const ackTime =
          new Date(r.first_acknowledged_at).getTime() -
          new Date(r.collected_at).getTime();
    ...
    const avgResponseTime =
      respondedCount > 0
        ? Math.round(totalResponseTime / respondedCount / (1000 * 60))
        : 0;
    const avgAckTime =
      acknowledgedCount > 0
        ? Math.round(totalAckTime / acknowledgedCount / (1000 * 60))
        : 0;
    const pendingResponses = reviewRows.filter(
      (r) => !r.responded_at && r.status !== "closed",
    ).length;
    ...
    for (const r of reviewRows) {
      ...
      if (
        ...
        Number(r.rating_normalized_10 ?? 10) <= 6
      )
        entry.neg += 1;
      if (r.rating_normalized_10 != null) {
        entry.totalRating += Number(r.rating_normalized_10);
      ...
      if (entry.topIssue === "N/A") {
        const reviewIssue = (issues ?? []).find(
          (iss) => iss.review_id === r.id,
        );
    ...
    const { data: properties } = await supabase
      .from("properties")
      .select("id,name")
      .in("id", Array.from(propertyCounts.keys()));
    ...
    const propertyRankings = Array.from(propertyCounts.entries())
      .map(([pid, entry]) => ({
        ...
        avgRating:
          entry.rated > 0
            ? Number((entry.totalRating / entry.rated).toFixed(1))
            : 0,
        ...
        avgResponseTime:
          entry.respondedCount > 0
            ? Math.round(
                entry.avgResponseTime / entry.respondedCount / (1000 * 60),
              )
            : 0,
      }))
    ...
    const negativeTrend = calculateTrend(
    ...
    let summaryEn =
      "A comprehensive analysis of guest feedback across all active properties for the reporting period.";
    let summaryAr =
      "تحليل شامل لتعليقات الضيوف عبر جميع الفنادق النشطة خلال فترة التقرير.";
    ...
    try {
      ...
      const recentReviewSamples = reviewRows.slice(0, 15).map((r) => ({
        date: (getReviewEventDate(r) ?? new Date(r.created_at))
          .toISOString()
          .split("T")[0],
        platform: r.platform,
      ...
      const aiResponse = await fetch(
        `${supabaseUrl}/functions/v1/process-ai-request`,
        {
          method: "POST",
          headers: {
...
Use ONLY real data from the reviews provided. Do not invent information.`,
        ...
        try {
          ...
          if (parsed.recommendations && Array.isArray(parsed.recommendations)) {
            recommendations = parsed.recommendations.map(
              (r: any) => `${r.priority} | ${r.hotel}: ${r.action}`,
            );
          }
        } catch (e) {
    ...
    const propertyTableHtml = propertyRankings
      .map((prop, index) => {
    ...
    const platformHtml = platformBreakdown
      .map(
        (p) => `
    ...
    const topIssuesHtml = topIssuesList
      .map(
        ([cat, count], index) => `
    ...
    const recommendationsHtml =
      recommendations.length > 0
        ? recommendations
            .map(
              (rec, i) =>
                `<li class="recommendation-item"><span class="rec-number">${i + 1}</span>${rec}</li>`,
            )
            .join("")
    ...
    const recentReviewsHtml = reviewRows
      .slice(0, 10)
      .map((r: any) => {
    ...
    const criticalAlertsHtml = criticalReviews
      .slice(0, 10)
      .map((r: any) => {
        // Use actual review text or summary - REAL DATA ONLY
        const excerpt = (
    ...
    const propertyScorecardsHtml = propertyRankings
      .map((prop, index) => {
        ...
        const posTrends =
          trends.positive.length > 0
            ? trends.positive
            : ["No significant positive trends identified"];
        const negTrends =
          trends.negative.length > 0
            ? trends.negative
            : ["No significant negative trends identified"];
    ...
    const operationalMatrixHtml = propertyRankings
      .map((prop) => {
        const trends = propertyTrends[prop.name];
      ...
      .insert({
        ...
        summary_json: {
          ...
          recent_reviews: reviewRows.slice(0, 15).map((r: any) => ({
            date: getReviewEventDate(r)?.toISOString() ?? r.created_at,
            platform: r.platform,
      ...
      await fetch(`${supabaseUrl}/functions/v1/send-email`, {
        ...
        body: JSON.stringify({
          ...
          variables: {
            ...
            recommendations_list:
              recommendations.length > 0
                ? recommendations
                    .map(
                      (r, i) =>
                        `<li class="rec-item"><span class="rec-num">${i + 1}</span>${r}</li>`,
                    )
                    .join("")
            ...
            top_performer_rating:
              propertyRankings[0]?.avgRating.toString() || "0",
            needs_attention:
              propertyRankings[propertyRankings.length - 1]?.name || "N/A",
          },
        }),
