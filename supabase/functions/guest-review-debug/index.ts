...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!serperRes.ok) {
      return new Response(
        JSON.stringify({ error: `Serper API failed: ${serperRes.status}` }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    if (reviews.length === 0) {
      return new Response(
        JSON.stringify({ error: "No reviews returned from Serper", data }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    const reviewPayload = {
      ...
      platform: "google",
      source_review_id:
        firstReview.reviewId || firstReview.id || `test-${Date.now()}`,
      ...
      reviewer_name:
        firstReview.user?.name || firstReview.reviewerName || "Anonymous",
