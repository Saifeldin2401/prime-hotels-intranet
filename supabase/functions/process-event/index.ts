...
const isAppRole = (value: unknown): value is AppRole => {
  return (
    typeof value === "string" && ALLOWED_CALLER_ROLES.has(value as AppRole)
  );
};
...
async function executeAction(
  ...
  const affectedUsers =
    context.affected_users || (context.user_id ? [context.user_id] : []);
  ...
  if (type === "assign_training") {
    ...
    const dueDays =
      typeof config.due_days === "number" ? config.due_days : null;
    ...
    const title = String(
      config.title ?? `Action required: ${context.event_type}`,
    );
    ...
    if (execError || !execution) {
      throw new Error(
        execError?.message || "Failed to create workflow execution",
      );
    }
...
async function applyAutoTrainingIfEnabled(
  ...
  const affectedUsers =
    payload.affected_users || (payload.user_id ? [payload.user_id] : []);
  ...
  const defaultDueDays =
    typeof configRecord?.default_due_days === "number"
      ? configRecord.default_due_days
      : 30;
  ...
  for (const userId of affectedUsers) {
    ...
    const matchingRules = rules.filter((rule) => {
      ...
      if (rule.target_department_id && !deptSet.has(rule.target_department_id))
        return false;
      if (rule.job_title_id && profile?.job_title_id !== rule.job_title_id)
        return false;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const departmentIds = (callerDepartments || []).map(
    ...
    const requestedUsers =
      rawAffectedUsers.length > 0
        ? rawAffectedUsers
        : payloadUserId && isUuid(payloadUserId)
          ? [payloadUserId]
          : [];
    ...
    if (!isHighPrivilege && requestedUsers.length > 0) {
      ...
      if (scopedAffectedUsers.length !== requestedUsers.length) {
        return new Response(
          JSON.stringify({
            error: "Forbidden: affected users outside your scope",
          }),
          {
            status: 403,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      try {
        ...
        const conditions: RuleCondition[] = Array.isArray(rule.conditions)
          ? rule.conditions.map((c: unknown) => ({
              ...
              operator: String(
                (c as Record<string, unknown>).operator ?? "equals",
              ) as RuleCondition["operator"],
