...
serve(async (req) => {
  ...
  try {
    ...
    const { model, prompt, task, temperature, max_tokens, maxOutputTokens } =
    ...
    if (forceMinimax) {
      ...
      if (!minimaxToken) {
        throw new Error(
          "FORCE_MINIMAX is enabled but HUGGINGFACE_MINIMAX_TOKEN is missing.",
        );
      }
      const minimaxModelId =
        Deno.env.get("HF_MINIMAX_MODEL_ID") || "MiniMaxAI/MiniMax-M2.5";
      console.warn(
        `FORCE_MINIMAX enabled. Routing request using HUGGINGFACE_MINIMAX_TOKEN with model ${minimaxModelId}`,
      );
    ...
    const effectiveMaxTokens = normalizeMaxTokens(
            ...
            messages: [
              {
                ...
                content:
                  currentTask === "summarization"
                    ? "You are an expert summarizer. Summarize the user input concisely into key bullet points."
                    : "You are a helpful assistant. Output valid JSON when requested.",
              },
    ...
    try {
      ...
      if (isQuotaOrCreditsError(status, message)) {
        console.warn(
          "Primary Hugging Face token quota/credits issue detected. Falling back to MiniMax-M2.5.",
        );
        const minimaxToken = Deno.env.get("HUGGINGFACE_MINIMAX_TOKEN");
        if (minimaxToken) {
          const minimaxModelId =
            Deno.env.get("HF_MINIMAX_MODEL_ID") || "MiniMaxAI/MiniMax-M2.5";
          console.warn(
            `Executing fallback call using HUGGINGFACE_MINIMAX_TOKEN with model ${minimaxModelId}`,
          );
    ...
    if (data?.error) {
      if (typeof data.error === "string" && data.error.includes("loading")) {
        throw new Error(
          `${targetModelId} is loading (cold start). Retry in 20s.`,
        );
      }
