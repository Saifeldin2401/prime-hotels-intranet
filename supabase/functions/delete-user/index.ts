...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (userIdRaw === user.id) {
      return new Response(JSON.stringify({ error: "You cannot permanently delete your own account." }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    const { data: targetUser, error: targetError } = await adminClient.auth.admin.getUserById(userIdRaw);
    if (targetError || !targetUser?.user) {
      return new Response(JSON.stringify({ error: "Target user not found in auth." }), {
        status: 404,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (deleteError) {
      return new Response(JSON.stringify({ error: `Failed to hard delete user: ${deleteError.message}` }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
