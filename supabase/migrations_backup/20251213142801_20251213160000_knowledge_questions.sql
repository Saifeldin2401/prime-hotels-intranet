...
-- ============================================================================
-- DEPENDENCY TABLES (Missing in some environments)
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_question_options_question ON knowledge_question_options(question_id);
...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_question_usages_question ON knowledge_question_usages(question_id);
CREATE INDEX IF NOT EXISTS idx_question_usages_entity ON knowledge_question_usages(usage_type, usage_entity_id);
...
CREATE INDEX IF NOT EXISTS idx_attempts_user ON knowledge_question_attempts(user_id);
CREATE INDEX IF NOT EXISTS idx_attempts_question ON knowledge_question_attempts(question_id);
CREATE INDEX IF NOT EXISTS idx_attempts_session ON knowledge_question_attempts(session_id);
CREATE INDEX IF NOT EXISTS idx_attempts_created ON knowledge_question_attempts(created_at);
CREATE INDEX IF NOT EXISTS idx_attempts_context ON knowledge_question_attempts(context_type, context_entity_id);
...
CREATE INDEX IF NOT EXISTS idx_versions_question ON knowledge_question_versions(question_id);
...
CREATE INDEX IF NOT EXISTS idx_sessions_user ON knowledge_quiz_sessions(user_id);
CREATE INDEX IF NOT EXISTS idx_sessions_quiz ON knowledge_quiz_sessions(quiz_type, quiz_entity_id);
...
DROP TRIGGER IF EXISTS question_updated ON knowledge_questions;
...
DROP TRIGGER IF EXISTS question_version_increment ON knowledge_questions;
...
DROP POLICY IF EXISTS "Published questions visible to all" ON knowledge_questions;
...
DROP POLICY IF EXISTS "Authenticated users can create questions" ON knowledge_questions;
...
DROP POLICY IF EXISTS "Options follow question visibility" ON knowledge_question_options;
...
DROP POLICY IF EXISTS "Usages follow question visibility" ON knowledge_question_usages;
...
CREATE POLICY "HR can manage usages" ON knowledge_question_usages
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM user_roles 
            WHERE user_id = auth.uid() 
            AND role IN ('regional_admin', 'regional_hr', 'property_hr', 'property_manager')
        )
    );
