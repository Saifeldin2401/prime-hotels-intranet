CREATE TABLE IF NOT EXISTS public.ai_agent_policies (
  ...
  routing_mode_override TEXT CHECK (routing_mode_override IS NULL OR routing_mode_override IN ('free_first','balanced','quality_first','premium')),
  force_model_id        TEXT,
  disabled_model_ids    TEXT[] NOT NULL DEFAULT ARRAY[]::text[],
  max_retries_override  INTEGER CHECK (max_retries_override IS NULL OR (max_retries_override BETWEEN 0 AND 5)),
  temperature_override  NUMERIC CHECK (temperature_override IS NULL OR (temperature_override BETWEEN 0 AND 2)),
  capability_override   TEXT CHECK (capability_override IS NULL OR capability_override IN ('structured_json','reasoning','fast','compliance','long_form')),
  notes                 TEXT,
  updated_at            TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_by            UUID
);

COMMENT ON TABLE public.ai_agent_policies IS 'Per-agent-role routing overrides for the multi-agent course engine (Gap D).';

INSERT INTO public.ai_agent_policies (agent_role) VALUES
  ('research'),('curriculum'),('knowledge'),('content_writer'),('activities'),
  ('scenarios'),('assessments'),('image_ai'),('video_ai'),('audio_ai'),
  ('qa_critic'),('revision'),('compliance'),('translator')
...
CREATE POLICY ai_agent_policies_select ON public.ai_agent_policies FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS ai_agent_policies_write ON public.ai_agent_policies;
CREATE POLICY ai_agent_policies_write ON public.ai_agent_policies FOR ALL TO authenticated
...
AS $function$
  SELECT coalesce(jsonb_agg(to_jsonb(p) ORDER BY p.agent_role), '[]'::jsonb)
