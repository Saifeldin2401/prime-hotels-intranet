...
-- 6. media_asset_usages  (COLUMN ONLY)
...
-- 7. media_collection_items  (COLUMN ONLY)
...
CREATE POLICY categories_manage_admin_insert ON public.categories
  ...
  WITH CHECK (public.org_visible(organization_id)
    AND (public.is_tenant_admin(organization_id) OR public.is_tenant_content_editor(organization_id) OR public.is_platform_super_admin()));
...
CREATE POLICY document_folders_delete ON public.document_folders
  FOR DELETE TO authenticated
  USING (public.org_visible(organization_id) AND is_system = false
    AND (has_role((SELECT auth.uid()), 'regional_admin'::app_role)
      OR (has_role((SELECT auth.uid()), 'property_manager'::app_role) AND property_id IS NOT NULL AND has_property_access((SELECT auth.uid()), property_id))
...
CREATE POLICY events_select_consolidated ON public.events
  FOR SELECT TO authenticated
  USING ((public.org_visible(organization_id) AND (is_public = true OR (SELECT auth.uid()) = ANY (attendees)))
    OR public.is_platform_super_admin());
CREATE POLICY "Users can create events" ON public.events
  FOR INSERT TO authenticated
  WITH CHECK (created_by = (SELECT auth.uid()) AND public.org_visible(organization_id));
CREATE POLICY "Users can update own events" ON public.events
  FOR UPDATE TO authenticated
  USING (created_by = (SELECT auth.uid()) AND public.org_visible(organization_id))
  WITH CHECK (created_by = (SELECT auth.uid()) AND public.org_visible(organization_id));
