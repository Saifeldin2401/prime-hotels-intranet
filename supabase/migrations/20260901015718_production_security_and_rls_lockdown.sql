-- PRODUCTION SECURITY & RLS LOCKDOWN (20260901260000)
BEGIN;
...
BEGIN
  ...
  SELECT ma.id, ma.title, ma.description, ma.filename, ma.public_url, ma.media_type, ma.category,
    ...
    COALESCE((SELECT jsonb_agg(jsonb_build_object(
      'id', mau.id, 'usage_type', mau.usage_type, 'usage_entity_id', mau.usage_entity_id,
      'usage_entity_title', mau.usage_entity_title, 'created_at', mau.created_at))
...
CREATE POLICY "media_asset_usages_tenant_select" ON public.media_asset_usages
  FOR SELECT TO authenticated USING (
    EXISTS (SELECT 1 FROM public.media_assets ma WHERE ma.id = media_asset_usages.media_asset_id
      AND (ma.is_public = true
        OR ma.organization_id = (SELECT auth.jwt() -> 'app_metadata' ->> 'organization_id')::uuid
        OR EXISTS (SELECT 1 FROM public.organization_memberships om WHERE om.user_id = (SELECT auth.uid()) AND om.organization_id = ma.organization_id AND om.is_active = true)
        OR EXISTS (SELECT 1 FROM public.platform_users pu WHERE pu.user_id = (SELECT auth.uid()) AND pu.is_active = true))));
...
CREATE POLICY "media_collection_items_tenant_select" ON public.media_collection_items
  FOR SELECT TO authenticated USING (
    EXISTS (SELECT 1 FROM public.media_collections mc WHERE mc.id = media_collection_items.collection_id
      AND (mc.organization_id = (SELECT auth.jwt() -> 'app_metadata' ->> 'organization_id')::uuid
        OR EXISTS (SELECT 1 FROM public.organization_memberships om WHERE om.user_id = (SELECT auth.uid()) AND om.organization_id = mc.organization_id AND om.is_active = true)
        OR EXISTS (SELECT 1 FROM public.platform_users pu WHERE pu.user_id = (SELECT auth.uid()) AND pu.is_active = true))));
