...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints
    WHERE table_schema='public' AND table_name='unified_question_options'
      AND constraint_name='unified_question_options_organization_id_fkey') THEN
    ALTER TABLE public.unified_question_options ADD CONSTRAINT unified_question_options_organization_id_fkey
      FOREIGN KEY (organization_id) REFERENCES public.organizations(id) ON DELETE CASCADE;
  END IF;
END $$;
UPDATE public.unified_question_options t SET organization_id = COALESCE(
...
CREATE TRIGGER trg_set_org BEFORE INSERT ON public.unified_question_options
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints
    WHERE table_schema='public' AND table_name='unified_question_usages'
      AND constraint_name='unified_question_usages_organization_id_fkey') THEN
    ALTER TABLE public.unified_question_usages ADD CONSTRAINT unified_question_usages_organization_id_fkey
      FOREIGN KEY (organization_id) REFERENCES public.organizations(id) ON DELETE CASCADE;
  END IF;
END $$;
UPDATE public.unified_question_usages t SET organization_id = COALESCE(
  (SELECT q.organization_id FROM public.unified_questions q WHERE q.id = t.question_id),
  'e0000000-0000-0000-0000-000000000001'::uuid) WHERE t.organization_id IS NULL;
...
CREATE TRIGGER trg_set_org BEFORE INSERT ON public.unified_question_usages
  FOR EACH ROW EXECUTE FUNCTION public.set_p5_org_from_parent('unified_questions', 'question_id');
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints
    WHERE table_schema='public' AND table_name='unified_question_versions'
      AND constraint_name='unified_question_versions_organization_id_fkey') THEN
    ALTER TABLE public.unified_question_versions ADD CONSTRAINT unified_question_versions_organization_id_fkey
      FOREIGN KEY (organization_id) REFERENCES public.organizations(id) ON DELETE CASCADE;
  END IF;
END $$;
UPDATE public.unified_question_versions t SET organization_id = COALESCE(
  (SELECT q.organization_id FROM public.unified_questions q WHERE q.id = t.question_id),
  'e0000000-0000-0000-0000-000000000001'::uuid) WHERE t.organization_id IS NULL;
...
CREATE TRIGGER trg_set_org BEFORE INSERT ON public.unified_question_versions
  FOR EACH ROW EXECUTE FUNCTION public.set_p5_org_from_parent('unified_questions', 'question_id');
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints
    WHERE table_schema='public' AND table_name='unified_quiz_questions'
      AND constraint_name='unified_quiz_questions_organization_id_fkey') THEN
    ALTER TABLE public.unified_quiz_questions ADD CONSTRAINT unified_quiz_questions_organization_id_fkey
      FOREIGN KEY (organization_id) REFERENCES public.organizations(id) ON DELETE CASCADE;
  END IF;
END $$;
UPDATE public.unified_quiz_questions t SET organization_id = COALESCE(
...
CREATE TRIGGER trg_set_org BEFORE INSERT ON public.unified_quiz_questions
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints
    WHERE table_schema='public' AND table_name='assessment_questions'
      AND constraint_name='assessment_questions_organization_id_fkey') THEN
    ALTER TABLE public.assessment_questions ADD CONSTRAINT assessment_questions_organization_id_fkey
      FOREIGN KEY (organization_id) REFERENCES public.organizations(id) ON DELETE CASCADE;
  END IF;
END $$;
UPDATE public.assessment_questions t SET organization_id = COALESCE(
...
CREATE TRIGGER trg_set_org BEFORE INSERT ON public.assessment_questions
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints
    WHERE table_schema='public' AND table_name='practical_submissions'
      AND constraint_name='practical_submissions_organization_id_fkey') THEN
    ALTER TABLE public.practical_submissions ADD CONSTRAINT practical_submissions_organization_id_fkey
      FOREIGN KEY (organization_id) REFERENCES public.organizations(id) ON DELETE CASCADE;
  END IF;
END $$;
UPDATE public.practical_submissions t SET organization_id = COALESCE(
...
CREATE TRIGGER trg_set_org BEFORE INSERT ON public.practical_submissions
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints
    WHERE table_schema='public' AND table_name='competency_levels'
      AND constraint_name='competency_levels_organization_id_fkey') THEN
    ALTER TABLE public.competency_levels ADD CONSTRAINT competency_levels_organization_id_fkey
      FOREIGN KEY (organization_id) REFERENCES public.organizations(id) ON DELETE CASCADE;
  END IF;
END $$;
UPDATE public.competency_levels t SET organization_id = COALESCE(
...
CREATE TRIGGER trg_set_org BEFORE INSERT ON public.competency_levels
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints
    WHERE table_schema='public' AND table_name='certificate_history'
      AND constraint_name='certificate_history_organization_id_fkey') THEN
    ALTER TABLE public.certificate_history ADD CONSTRAINT certificate_history_organization_id_fkey
      FOREIGN KEY (organization_id) REFERENCES public.organizations(id) ON DELETE CASCADE;
  END IF;
END $$;
UPDATE public.certificate_history t SET organization_id = COALESCE(
...
CREATE TRIGGER trg_set_org BEFORE INSERT ON public.certificate_history
  ...
  USING (
    ...
    AND (
      ...
      OR EXISTS (SELECT 1 FROM public.certificates c
        WHERE c.id = certificate_history.certificate_id AND c.user_id = (SELECT auth.uid()))
    )
  );
