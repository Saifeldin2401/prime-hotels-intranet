...
-- Correcting my own oversight in the previous migration: search_documents does its own
-- full manual visibility check in the WHERE clause (org_visible / is_master_template /
-- created_by / owner_id / status+visibility), exactly like its sibling functions
-- (search_sops, fuzzy_search_documents, secure_search_documents), all of which are
-- SECURITY DEFINER. I rewrote this function from `prosrc` alone without re-checking its
-- original header and it landed as SECURITY INVOKER (prosecdef=false), which would let
-- normal table RLS silently interact with/override the function's own hand-built logic
-- (e.g. potentially hiding is_master_template rows from other orgs that the manual check
-- was explicitly written to allow). Restoring STABLE SECURITY DEFINER to match its siblings.
