...
-- ---- §48: per-tenant identity providers (SSO) ----
CREATE TABLE IF NOT EXISTS public.identity_providers (
  ...
  email_domain text,                      -- auto-route logins from @this-domain to this IdP
  config jsonb NOT NULL DEFAULT '{}'::jsonb,   -- metadata_url, client_id, entity_id, acs_url, jit_provisioning flags... (NO secrets in plaintext)
...
CREATE TABLE IF NOT EXISTS public.api_keys (
  ...
  key_hash text NOT NULL,                 -- sha256 of the full key; raw key shown once at creation
  scopes text[] NOT NULL DEFAULT '{}',    -- e.g. training:read, users:write
...
-- ---- §49: event outbox + webhooks ----
CREATE TABLE IF NOT EXISTS public.platform_events (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  event_type text NOT NULL,               -- user.created, training.assigned, assessment.passed, certification.expiring, organization.suspended, ...
  organization_id uuid REFERENCES public.organizations(id) ON DELETE CASCADE,
  actor_id uuid,
  resource_type text,
  resource_id text,
...
-- ---- RLS: all tenant-scoped; secrets never selectable by non-admins ----
...
-- writes only via SECURITY DEFINER emit_platform_event()
...
GRANT SELECT ON public.identity_providers, public.platform_events, public.webhook_endpoints, public.webhook_deliveries TO authenticated;
