-- ============================================================================
-- MIGRATION: replace_simple_projects_with_capex_module
-- Supersedes the simpler add_projects_module migration (projects table +
-- tasks.project_id reuse) with the more complete, purpose-built CAPEX/
-- pre-opening design found in the (never-applied) local migration file
-- 20260727022238_add_capex_projects_module.sql: capex_projects, milestones,
-- expenditures (with auto-rollup), pre_opening_checklist_items, and
-- reusable templates. Zero data loss -- the superseded projects table and
-- tasks.project_id column both had 0 real rows (only rolled-back test data).
-- ============================================================================

ALTER TABLE public.tasks DROP COLUMN project_id;
DROP TABLE public.projects;

-- create_task_atomic: drop the project_id handling (column no longer exists),
-- keep the two real bug fixes from moments ago (nonexistent `assigned_to`
-- column, missing task_priority enum cast) -- those are independent of which
-- Projects schema is in use.
...
-- Now apply the capex_projects module verbatim from the local file.
