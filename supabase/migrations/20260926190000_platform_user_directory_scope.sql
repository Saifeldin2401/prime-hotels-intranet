-- Migration: 20260926190000_platform_user_directory_scope.sql
-- Enforce strict database-level scoping for customer members vs platform operators
-- in get_platform_user_directory RPC.

DROP FUNCTION IF EXISTS public.get_platform_user_directory(text, uuid, text, integer, integer);
DROP FUNCTION IF EXISTS public.get_platform_user_directory(text, uuid, text, integer, integer, text);
...
CREATE OR REPLACE FUNCTION public.get_platform_user_directory(
  ...
  p_member_scope text DEFAULT 'customers_only'::text
)
RETURNS TABLE(
  id uuid,
  email text,
  ...
  filtered AS (
    ...
    WHERE (p_search IS NULL OR p.full_name ILIKE '%'||p_search||'%' OR p.email ILIKE '%'||p_search||'%')
      ...
      AND (
        p_member_scope = 'all'
        OR (
          p_member_scope = 'customers_only'
          AND NOT public.is_platform_operator(p.id)
          AND EXISTS (SELECT 1 FROM public.organization_memberships om_check WHERE om_check.user_id = p.id)
        )
        OR (
          p_member_scope = 'operators_only'
          AND public.is_platform_operator(p.id)
        )
      )
  )
...
GRANT EXECUTE ON FUNCTION public.get_platform_user_directory(text, uuid, text, integer, integer, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_platform_user_directory(text, uuid, text, integer, integer, text) TO service_role;
