-- Production-readiness cleanup (learning-platform scope). See repo file
-- supabase/migrations/20260921090000_production_readiness_cleanup.sql for full commentary.
...
CREATE POLICY tasks_update ON public.tasks FOR UPDATE TO authenticated
  USING (public.org_visible(organization_id) AND (created_by_id = (SELECT auth.uid()) OR assigned_to_id = (SELECT auth.uid()) OR public.is_tenant_admin(organization_id)))
  WITH CHECK (public.org_visible(organization_id) AND (created_by_id = (SELECT auth.uid()) OR assigned_to_id = (SELECT auth.uid()) OR public.is_tenant_admin(organization_id)));
DROP POLICY IF EXISTS tasks_delete ON public.tasks;
CREATE POLICY tasks_delete ON public.tasks FOR DELETE TO authenticated
  USING (public.org_visible(organization_id) AND (created_by_id = (SELECT auth.uid()) OR public.is_tenant_admin(organization_id)));

DROP TABLE IF EXISTS
  public.api_keys, public.service_accounts, public.conversation_participants,
  public.data_import_logs, public.identity_providers, public.knowledge_required_reading,
  public.learning_events, public.lesson_progress, public.lesson_blocks,
  public.message_attachments, public.objective_links, public.scheduled_report_executions,
  public.training_certificate_settings, public.training_module_prerequisites,
