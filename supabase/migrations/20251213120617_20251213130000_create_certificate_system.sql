...
-- Check if function exists before dropping to avoid errors (not needed for CREATE OR REPLACE)
-- Function to generate unique certificate number - SECURITY DEFINER required for RLS sequence
...
DROP TRIGGER IF EXISTS trigger_set_certificate_identifiers ON certificates;
...
DROP POLICY IF EXISTS "Managers can view property certificates" ON certificates;
DROP POLICY IF EXISTS "Regional admins can view all certificates" ON certificates;
DROP POLICY IF EXISTS "Service can insert certificates" ON certificates;
DROP POLICY IF EXISTS "Users can view own certificate history" ON certificate_history;
DROP POLICY IF EXISTS "Admins can view all certificate history" ON certificate_history;
...
-- OPEN insert policy for authenticated users (required for creating own certificates via script/ui)
