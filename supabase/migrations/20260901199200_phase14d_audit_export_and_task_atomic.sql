...
-- Phase 14d.
-- get_audit_data_for_export: the corporate_admin/compliance_officer branch returned every
-- org's system_events audit rows + auth.users.email with no tenant scoping. system_events has
-- no organization_id, so restrict to platform super admin until per-tenant audit export is
-- built properly.
...
-- create_task_atomic: tasks is a purged domain (frontend removed, table pending deletion).
-- The function allowed cross-tenant task injection + a notification row to an arbitrary
-- user_id (in-app phishing). Revoke from client roles.
