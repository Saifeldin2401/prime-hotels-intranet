...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (SELECT 1 FROM public.platform_users pu WHERE pu.user_id = _user_id AND pu.is_active);
$$;
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT (SELECT legacy_role_fallback_enabled FROM public.platform_config WHERE id)
     AND EXISTS (
       ...
       WHERE user_id = _user_id AND role IN ('super_admin','corporate_admin','administrator')
     );
$$;

CREATE OR REPLACE FUNCTION public.is_platform_user(target_user_id uuid DEFAULT auth.uid())
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT public.is_platform_operator(target_user_id) OR public._legacy_platform_fallback(target_user_id);
$$;

CREATE OR REPLACE FUNCTION public.is_platform_super_admin()
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT public.is_platform_operator(auth.uid()) OR public._legacy_platform_fallback(auth.uid());
$$;
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT public.platform_operator_has_role('platform_admin', _user_id)
      OR ( (SELECT legacy_role_fallback_enabled FROM public.platform_config WHERE id)
           AND EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = _user_id AND role = 'administrator') );
$$;
...
BEGIN
  ...
  END IF;
  UPDATE public.platform_access_sessions SET is_active = false, ended_at = now()
...
REVOKE ALL ON FUNCTION
  public.is_platform_operator(uuid), public.platform_operator_has_role(text,uuid),
  public.platform_operator_can(text,uuid), public._legacy_platform_fallback(uuid)
FROM anon, public;
...
CREATE POLICY competency_levels_write ON public.competency_levels FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM public.competencies c
                 WHERE c.id = competency_levels.competency_id
                   AND public.org_visible(c.organization_id) AND public.is_tenant_content_editor(c.organization_id)))
  ...
  USING (user_id = (SELECT auth.uid())
         OR EXISTS (SELECT 1 FROM public.training_sessions ts
                    WHERE ts.id = training_session_attendees.session_id
                      AND public.org_visible(ts.organization_id) AND public.is_tenant_content_editor(ts.organization_id)));
CREATE POLICY training_session_attendees_write ON public.training_session_attendees FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM public.training_sessions ts
                 WHERE ts.id = training_session_attendees.session_id
                   AND public.org_visible(ts.organization_id) AND public.is_tenant_people_admin(ts.organization_id)))
...
DROP POLICY IF EXISTS practical_submissions_del    ON public.practical_submissions;
  ...
  USING (learner_id = (SELECT auth.uid())
         OR EXISTS (SELECT 1 FROM public.practical_assessments pa
                    WHERE pa.id = practical_submissions.assessment_id
                      AND public.org_visible(pa.organization_id) AND public.is_tenant_content_editor(pa.organization_id)));
...
BEGIN
  ...
  LIMIT 1;
  IF v_is_operator THEN v_dest := '/platform';
  ELSIF v_top_role IN ('organization_owner','organization_admin','brand_admin','hotel_admin') THEN v_dest := '/admin';
  ELSIF v_top_role IN ('training_manager','instructor') THEN v_dest := '/training';
  ELSIF v_top_role IN ('knowledge_manager','author') THEN v_dest := '/knowledge';
  ELSE v_dest := '/home/learner'; END IF;
  RETURN jsonb_build_object(
    'user_id', v_uid, 'is_platform_operator', v_is_operator,
...
WHERE p.email IN ('islam.mahrous@gmail.com','admin@prime.com','hsmadi2223@gmail.com','yousef.buobaid@gmail.com')
...
FROM want w JOIN public.profiles p ON p.email = w.email
WHERE NOT EXISTS (
  SELECT 1 FROM public.platform_role_assignments x
  WHERE x.platform_user_id = p.id AND x.platform_role = w.role::public.platform_role AND x.revoked_at IS NULL
);
