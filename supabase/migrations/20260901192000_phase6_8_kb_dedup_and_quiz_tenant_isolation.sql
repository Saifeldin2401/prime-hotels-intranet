...
-- Phase 8: tenant-isolate learning_quizzes (20 live rows).
...
UPDATE public.learning_quizzes lq
   SET organization_id = COALESCE(tm.organization_id, 'e0000000-0000-0000-0000-000000000001')
  FROM public.training_modules tm
 WHERE lq.training_module_id = tm.id AND lq.organization_id IS NULL;
...
USING (public.org_visible(organization_id)
       AND (public.is_tenant_content_editor(organization_id) OR public.is_content_author() OR public.is_training_manager() OR created_by = auth.uid()))
WITH CHECK (public.org_visible(organization_id)
       AND (public.is_tenant_content_editor(organization_id) OR public.is_content_author() OR public.is_training_manager()));

-- Phase 8: defense-in-depth org_id on the per-user quiz session/attempt tables.
