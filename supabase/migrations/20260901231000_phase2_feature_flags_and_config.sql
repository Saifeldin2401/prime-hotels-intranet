...
BEGIN
  v_org := COALESCE(
    p_org_id, public.get_operator_impersonated_org(),
    (SELECT organization_id FROM public.organization_memberships
     WHERE user_id = auth.uid() AND is_active = true ORDER BY is_primary DESC LIMIT 1)
  );
...
BEGIN
  ...
  END IF;
  UPDATE public.platform_feature_flags SET default_enabled = p_enabled, updated_at = now(), updated_by = auth.uid()
   WHERE key = p_key;
...
CREATE POLICY "settings_insert_operators_and_admins" ON public.system_settings
  ...
  WITH CHECK (
    ...
    OR EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = (SELECT auth.uid()) AND role IN ('corporate_admin','regional_admin'))
  );
