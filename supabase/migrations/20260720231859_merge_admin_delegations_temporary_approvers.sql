...
-- Both tables were verified 0 rows before this change. See prior failed attempt in this
-- session for full design rationale; re-stated briefly:
--  * delegation_category ('admin' | 'temporary_approval') is the new discriminator.
--    Not named delegation_type because that name is already used by admin_delegations
--    for its own sub-classification ('full_access'|'specific_permissions'|'approval_authority'),
--    which is deeply embedded in src/pages/admin/DelegationSettings.tsx.
--  * starts_at/ends_at (admin_delegations' naming) replaces temporary_approvers' start_at/end_at.
--  * The no-overlap EXCLUDE constraint (previously admin_delegations only) stays scoped
--    to delegation_category = 'admin' so it doesn't newly restrict temporary-approval rows.
--
