-- Atomic Operations Refined (2025-12-16)
-- Improved to derive Notification User ID from the entity itself, avoiding client round-trip.

-- 1. Approve Leave Request Atomic
CREATE OR REPLACE FUNCTION approve_leave_request(
  request_id UUID,
  approver_id UUID,
  notification_payload JSONB DEFAULT NULL -- Keys: type, title, message, link, data
) RETURNS JSONB AS $$
DECLARE
...
BEGIN
  ...
  IF notification_payload IS NOT NULL AND v_request.requester_id IS NOT NULL THEN
    ...
    VALUES (
      v_request.requester_id, -- Derived from request
