...
BEGIN
  FOREACH t IN ARRAY ARRAY['system_events','analytics_events','ai_usage_log','search_logs','user_sessions'] LOOP
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = t || '_organization_id_fkey' AND conrelid = ('public.'||t)::regclass) THEN
...
UPDATE public.system_events se SET organization_id = COALESCE(
  (SELECT om.organization_id FROM public.organization_memberships om WHERE om.user_id = se.actor_id AND om.is_active ORDER BY om.is_primary DESC, om.created_at ASC LIMIT 1),
  'e0000000-0000-0000-0000-000000000001'::uuid) WHERE se.organization_id IS NULL;
UPDATE public.analytics_events ae SET organization_id = COALESCE(
  (SELECT om.organization_id FROM public.organization_memberships om WHERE om.user_id = ae.user_id AND om.is_active ORDER BY om.is_primary DESC, om.created_at ASC LIMIT 1),
  'e0000000-0000-0000-0000-000000000001'::uuid) WHERE ae.organization_id IS NULL;
UPDATE public.ai_usage_log al SET organization_id = COALESCE(
  (SELECT om.organization_id FROM public.organization_memberships om WHERE om.user_id = al.user_id AND om.is_active ORDER BY om.is_primary DESC, om.created_at ASC LIMIT 1),
  (SELECT tm.organization_id FROM public.training_modules tm WHERE tm.id = al.course_id),
  'e0000000-0000-0000-0000-000000000001'::uuid) WHERE al.organization_id IS NULL;
UPDATE public.search_logs sl SET organization_id = COALESCE(
  (SELECT om.organization_id FROM public.organization_memberships om WHERE om.user_id = sl.user_id AND om.is_active ORDER BY om.is_primary DESC, om.created_at ASC LIMIT 1),
  ...
  'e0000000-0000-0000-0000-000000000001'::uuid) WHERE sl.organization_id IS NULL;
UPDATE public.user_sessions us SET organization_id = COALESCE(
  (SELECT om.organization_id FROM public.organization_memberships om WHERE om.user_id = us.user_id AND om.is_active ORDER BY om.is_primary DESC, om.created_at ASC LIMIT 1),
  'e0000000-0000-0000-0000-000000000001'::uuid) WHERE us.organization_id IS NULL;
...
AS $$
DECLARE v_actor uuid; v_org uuid; LIT constant uuid := 'e0000000-0000-0000-0000-000000000001';
BEGIN
  IF NEW.organization_id IS NOT NULL THEN RETURN NEW; END IF;
  IF TG_TABLE_NAME = 'system_events' THEN v_actor := NEW.actor_id; ELSE v_actor := NEW.user_id; END IF;
  IF v_actor IS NOT NULL THEN
    SELECT om.organization_id INTO v_org FROM public.organization_memberships om
    WHERE om.user_id = v_actor AND om.is_active ORDER BY om.is_primary DESC, om.created_at ASC LIMIT 1;
  END IF;
...
BEGIN
  FOREACH t IN ARRAY ARRAY['system_events','analytics_events','ai_usage_log','search_logs','user_sessions'] LOOP
    ...
    EXECUTE format('CREATE TRIGGER trg_audit_set_organization_id BEFORE INSERT ON public.%I FOR EACH ROW EXECUTE FUNCTION public.tg_audit_set_organization_id()', t);
  END LOOP;
END $$;
...
CREATE POLICY analytics_events_insert_own ON public.analytics_events
  ...
  WITH CHECK (user_id IS NULL OR user_id = (SELECT auth.uid()) OR (SELECT auth.role()) = 'service_role');
...
CREATE POLICY search_logs_insert ON public.search_logs
  FOR INSERT TO authenticated, service_role
  WITH CHECK (user_id = (SELECT auth.uid()) OR user_id IS NULL OR (SELECT auth.role()) = 'service_role');
...
CREATE POLICY user_sessions_manage_tenant ON public.user_sessions
  ...
  USING ((public.org_visible(organization_id) AND public.is_tenant_admin(organization_id)) OR public.is_platform_super_admin())
  WITH CHECK ((public.org_visible(organization_id) AND public.is_tenant_admin(organization_id)) OR public.is_platform_super_admin());
