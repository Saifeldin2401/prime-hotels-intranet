...
BEGIN
  ...
  IF v_property_id IS NOT NULL AND NOT EXISTS (
    ...
    -- Fallback: check if has_property_access function exists and works
    -- But for now, let's keep it simple and direct
  ...
  VALUES (
    ...
    (COALESCE(task_data->>'status', 'open'))::entity_status, -- Corrected type
    (COALESCE(task_data->>'priority', 'medium')), -- Corrected to text
    v_assigned_to,
    v_created_by,
    v_property_id,
