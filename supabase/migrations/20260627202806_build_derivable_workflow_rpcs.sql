...
-- Next recurrence timestamp for a recurring task template.
...
-- Delete a data import log (admin/manager only).
...
-- Enqueue a manual run of a scheduled report; returns the run id.
...
-- Approve / reject a pending self-signup user.
...
-- Employee request for new knowledge-base content (recorded as a system event for HR/admin triage).
