...
-- ============================================================================
-- §55 — per-tenant AI cost control (hard enforcement, not just 80/90/100% warnings)
-- §37/§38 — competency requirements + skills-gap engine
-- ============================================================================

-- ---- §55: AI job metering columns + enforcement ----------------------------
...
-- resolve the org for a job: explicit, else the creator's primary membership
...
BEGIN
  ...
  IF v_org IS NOT NULL AND NOT public.check_ai_credit(v_org) THEN
    RAISE EXCEPTION 'Monthly AI credit limit reached for this subscription plan (%). Contact your platform administrator.',
      (SELECT max_ai_credits_monthly FROM public.organizations WHERE id = v_org)
      USING ERRCODE = '23514';
  END IF;
  RETURN NEW;
END;
...
-- monthly reset (call from pg_cron on the 1st)
...
GRANT EXECUTE ON FUNCTION public.consume_ai_credit(uuid,integer,bigint,numeric) TO service_role;
GRANT EXECUTE ON FUNCTION public.reset_monthly_ai_credits() TO service_role;

-- ---- §37/§38: competency requirements + skill-gap engine -------------------
CREATE TABLE IF NOT EXISTS public.role_competency_requirements (
  id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  organization_id uuid NOT NULL REFERENCES public.organizations(id) ON DELETE CASCADE,
  membership_role text NOT NULL,            -- membership_role value the requirement applies to
  department_id   uuid REFERENCES public.departments(id) ON DELETE CASCADE,  -- NULL = org-wide for that role
...
-- Skill gaps for one learner: required (by their role/dept) vs current level,
-- plus the courses that develop each gapped competency.
...
-- Department skill matrix: every member x every required competency for the dept.
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'competencies', COALESCE((
      ...
      FROM public.role_competency_requirements r
    ...
    'members', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'user_id', m.user_id, 'name', p.full_name, 'role', m.role::text,
        'levels', (
          SELECT COALESCE(jsonb_object_agg(uc.competency_id, uc.current_level), '{}'::jsonb)
          FROM public.user_competencies uc WHERE uc.user_id = m.user_id
        )
      ))
      FROM public.organization_memberships m
      JOIN public.profiles p ON p.id = m.user_id
      WHERE m.department_id = p_department_id AND m.is_active = true
    ), '[]'::jsonb)
  ) INTO v;
  RETURN v;
END;
