-- Fix: prevent an actor from approving/rejecting/returning/forwarding their own request.
--
-- request_apply_action's authorization check for approve/reject/return/forward bypasses the
-- assignee check for anyone holding the HR or admin role, but never verified that the actor
-- isn't the REQUESTER of the request they're acting on. This let an HR/admin employee submit
-- their own leave request, promotion, transfer, etc. and then approve/reject/return/forward
-- their own pending step, since holding an HR/admin role alone satisfied the gate regardless
-- of whose request it was.
--
-- This migration adds a universal self-approval guard (applies to all entity_types) right
-- after the existing assignee/is_hr/is_admin check, and is additive to the promotion
-- role-hierarchy check and the close-action restriction added earlier this session. All other
-- logic, the six action branches, and the parameter defaults are preserved exactly.
