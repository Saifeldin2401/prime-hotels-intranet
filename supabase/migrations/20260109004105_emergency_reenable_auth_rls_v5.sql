-- Emergency Fix: Re-enable RLS on Auth Tables with Proper Security (v5)
-- Date: 2026-01-09
-- Purpose: Fix critical security vulnerability where RLS was disabled on auth tables
-- Changes in v5: Matched all function signatures and types, omitted plural get_user_roles
...
CREATE POLICY "Users can view own properties"
  ...
  USING (
    ...
    OR EXISTS (
      SELECT 1 FROM public.user_roles ur
      WHERE ur.user_id = auth.uid()
      AND ur.role IN ('regional_admin'::app_role, 'regional_hr'::app_role)
    )
  );
...
CREATE POLICY "Users can view own departments"
  ...
  USING (
    ...
    OR EXISTS (
      SELECT 1 FROM public.user_roles ur
      WHERE ur.user_id = auth.uid()
      AND ur.role IN ('regional_admin'::app_role, 'regional_hr'::app_role, 'property_manager'::app_role)
    )
  );
...
CREATE POLICY "Users can view profiles in scope"
  ...
  USING (
    ...
    OR EXISTS (
      -- Users in same property can view each other
      SELECT 1 
      FROM public.user_properties up1
      JOIN public.user_properties up2 ON up1.property_id = up2.property_id
...
CREATE POLICY "Admins can manage profiles"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM public.user_roles ur
      WHERE ur.user_id = auth.uid()
      AND ur.role IN ('regional_admin'::app_role, 'regional_hr'::app_role, 'property_hr'::app_role)
    )
  );
