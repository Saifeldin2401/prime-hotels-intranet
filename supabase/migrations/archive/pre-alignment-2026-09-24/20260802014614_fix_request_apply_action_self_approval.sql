-- ============================================================================
-- MIGRATION: fix_request_apply_action_self_approval
-- request_apply_action() -- the generic approve/reject/forward/close handler
-- backing leave/promotion/transfer/expense-claim routing -- only checked
-- can_view_request(p_request_id), which returns TRUE for the requester
-- themselves (by design, so they can see their own request's status). It
-- never separately verified that the caller is the actually-assigned
-- approver (current_step.assignee_id) before mutating the pending step. Any
-- requester could call this on their own pending request with
-- p_action = 'approve' and self-approve their own leave/promotion/transfer,
-- fully bypassing the assigned approver.
--
-- Fix: for the step-mutating actions (approve/reject/return/forward), also
-- require auth.uid() = current_step.assignee_id, or that the caller is
-- HR/admin (is_hr/is_admin -- matching the helpers can_view_request already
