...
DO $$ BEGIN
  ...
  DROP POLICY IF EXISTS "Assigned users can update tickets" ON maintenance_tickets;
END $$;
...
DO $$ BEGIN
  ...
  DROP POLICY IF EXISTS "Managers can view leave requests for their department" ON leave_requests;
...
DO $$ BEGIN
  DROP POLICY IF EXISTS "Public read access for published jobs" ON job_postings;
