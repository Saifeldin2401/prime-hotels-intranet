...
BEGIN
  ...
  -- Handle empty due_date string (convert to NULL)
  v_due_date_str := task_data->>'due_date';
  IF v_due_date_str IS NOT NULL AND v_due_date_str != '' THEN
    v_due_date := v_due_date_str::TIMESTAMPTZ;
  ELSE
    v_due_date := NULL;
  END IF;
  ...
  IF notification_payload IS NOT NULL AND v_assigned_to IS NOT NULL AND v_assigned_to != v_created_by THEN
     ...
     VALUES (
      v_assigned_to,
      (notification_payload->>'type')::notification_type,
