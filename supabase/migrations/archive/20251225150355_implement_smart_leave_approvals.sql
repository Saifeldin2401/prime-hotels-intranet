-- Enhanced Leave Request Trigger with Smart Approvals
...
BEGIN
  ...
  -- Criteria: sick or annual leave, AND duration <= 2 days
  IF NEW.type IN ('sick', 'annual') AND v_duration <= 2 THEN
    v_is_smart_eligible := TRUE;
  END IF;

  -- 3. Resolve Approvers (Even if auto-approving, we might want to know who WOULD have approved)
  ...
  SET 
    ...
    status = CASE WHEN v_is_smart_eligible THEN 'approved'::leave_request_status ELSE 'pending'::leave_request_status END,
    approved_by_id = CASE WHEN v_is_smart_eligible THEN auth.uid() ELSE NULL END -- System approve is usually service_role but auth.uid() here is fine
  WHERE id = NEW.id;
  ...
  IF v_is_smart_eligible THEN
    -- For smart approval, create one completed system step
  ...
  ELSE
    -- Standard notification to approver
