CREATE TABLE IF NOT EXISTS sop_documents (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), title TEXT NOT NULL, content TEXT, version INTEGER DEFAULT 1, status TEXT NOT NULL DEFAULT 'draft' CHECK (status IN ('draft', 'pending_review', 'approved', 'published', 'archived')), category TEXT, property_id UUID REFERENCES properties(id) ON DELETE SET NULL, department_id UUID REFERENCES departments(id) ON DELETE SET NULL, created_by UUID REFERENCES profiles(id) ON DELETE SET NULL, approved_by UUID REFERENCES profiles(id) ON DELETE SET NULL, published_at TIMESTAMPTZ, requires_quiz BOOLEAN DEFAULT FALSE, passing_score INTEGER DEFAULT 70, quiz_enabled BOOLEAN DEFAULT FALSE, created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW());
CREATE TABLE IF NOT EXISTS sop_assignments (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), sop_document_id UUID REFERENCES sop_documents(id) ON DELETE CASCADE, assigned_to_user_id UUID REFERENCES profiles(id) ON DELETE CASCADE, assigned_by UUID REFERENCES profiles(id) ON DELETE SET NULL, due_date DATE, completed BOOLEAN DEFAULT FALSE, completed_at TIMESTAMPTZ, created_at TIMESTAMPTZ DEFAULT NOW(), UNIQUE(sop_document_id, assigned_to_user_id));
CREATE TABLE IF NOT EXISTS sop_reading_logs (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), sop_document_id UUID REFERENCES sop_documents(id) ON DELETE CASCADE, user_id UUID REFERENCES profiles(id) ON DELETE CASCADE, read_at TIMESTAMPTZ DEFAULT NOW(), completed BOOLEAN DEFAULT FALSE, UNIQUE(sop_document_id, user_id));
CREATE TABLE IF NOT EXISTS sop_quiz_questions (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), sop_document_id UUID REFERENCES sop_documents(id) ON DELETE CASCADE, question_text TEXT NOT NULL, question_type TEXT NOT NULL CHECK (question_type IN ('mcq', 'true_false', 'fill_blank')), options JSONB, correct_answer TEXT NOT NULL, points INTEGER DEFAULT 1, order_index INTEGER DEFAULT 0, created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW());
CREATE TABLE IF NOT EXISTS sop_quiz_attempts (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), sop_document_id UUID REFERENCES sop_documents(id) ON DELETE CASCADE, user_id UUID REFERENCES profiles(id) ON DELETE CASCADE, score DECIMAL NOT NULL, total_points INTEGER NOT NULL, percentage DECIMAL NOT NULL, passed BOOLEAN NOT NULL, answers JSONB NOT NULL, started_at TIMESTAMPTZ DEFAULT NOW(), completed_at TIMESTAMPTZ, certificate_url TEXT, created_at TIMESTAMPTZ DEFAULT NOW());
