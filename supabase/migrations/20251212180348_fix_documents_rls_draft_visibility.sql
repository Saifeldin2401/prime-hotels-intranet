-- Drop the existing policy and recreate with proper draft visibility
...
-- DRAFTS: Only visible to authors and approvers
...
USING (
  ...
  -- Property managers see documents for their properties
  (public.has_role(auth.uid(), 'property_manager') AND 
   public.has_property_access(auth.uid(), property_id)) OR
  -- Property HR see documents for their properties
  (public.has_role(auth.uid(), 'property_hr') AND 
   public.has_property_access(auth.uid(), property_id)) OR
  -- Department heads see documents for their departments
