...
-- create_scoped_training_assignment referenced a non-existent identifier
-- `p_eligible_users` in the training_assignment_rules INSERT (should be the
-- local `v_eligible_users`). Any successful assignment path therefore errored
-- with `column "p_eligible_users" does not exist`; only the direct-write and
-- trigger paths ever populated training_assignment_rules. Same failure class as
-- the historical create_task_atomic / apply_request_step_sla bugs.
...
BEGIN
  ...
  SELECT title INTO v_course_title FROM public.training_modules
  WHERE id = p_course_id AND is_deleted IS NOT TRUE;
  IF v_course_title IS NULL THEN
    SELECT title INTO v_course_title FROM public.courses
    WHERE id = p_course_id AND is_deleted IS NOT TRUE;
  END IF;
  IF v_course_title IS NULL THEN
  ...
  IF p_scope_type = 'individual' AND p_target_user_ids IS NOT NULL THEN
    SELECT ARRAY(
      SELECT DISTINCT om.user_id FROM public.organization_memberships om
  ...
  INSERT INTO public.training_assignment_rules (
    training_module_id, content_id, content_type, organization_id, brand_id, hotel_id,
    department_id, target_role, target_type, target_id, scope_type, scope_id,
    target_user_ids, recipient_count, due_date, priority, instructions,
    requires_acknowledgement, notify_on_due, reminder_days_before, assigned_by,
    created_by, is_active, status
  ) VALUES (
    p_course_id, p_course_id, 'module', p_organization_id, p_brand_id, p_hotel_id,
    p_department_id, p_target_role, p_scope_type,
    ...
    p_requires_acknowledgement, p_notify_on_due, p_reminder_days_before, v_caller_id,
    v_caller_id, true, 'active'
  )
  RETURNING id INTO v_rule_id;

  FOREACH v_user_id IN ARRAY v_eligible_users LOOP
    INSERT INTO public.training_progress (
      user_id, training_id, assignment_id, organization_id, lp_content_type, status, created_at, updated_at
    ) VALUES (
      v_user_id, p_course_id, v_rule_id, p_organization_id, 'module', 'not_started'::training_status, now(), now()
    )
    ON CONFLICT (user_id, training_id) DO UPDATE SET
      assignment_id = EXCLUDED.assignment_id, organization_id = EXCLUDED.organization_id, updated_at = now();

    INSERT INTO public.enrollments (
      user_id, course_id, assignment_id, organization_id, status, enrolled_at, created_at, updated_at
    ) VALUES (
      v_user_id, p_course_id, v_rule_id, p_organization_id, 'enrolled'::enrollment_status, now(), now(), now()
    )
    ...
    VALUES (
      v_user_id, 'New Training Assigned: ' || v_course_title,
      COALESCE(p_instructions, 'You have been assigned to complete "' || v_course_title || '".'),
      'training_assigned', '/training/player/' || p_course_id, false, now()
    );
  END LOOP;
  ...
  IF (v_scopes->>'is_platform_admin')::boolean IS TRUE THEN
    ...
    VALUES (
      v_caller_id, 'create_training_assignment', 'training_assignment_rules', v_rule_id, p_organization_id,
