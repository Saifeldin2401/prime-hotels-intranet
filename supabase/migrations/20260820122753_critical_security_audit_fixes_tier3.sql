...
BEGIN
  ...
  IF v_module.validity_period_days IS NOT NULL AND v_module.validity_period_days > 0 THEN
    v_expiry := coalesce(v_tp.completed_at, now()) + make_interval(days => v_module.validity_period_days);
  END IF;
  ...
  INSERT INTO public.certificates (
    ...
    'training', v_cert_number, v_verification_code, coalesce(v_module.title, 'Training Module'),
    coalesce(v_tp.completed_at, now()), v_expiry,
    v_tp.score_percentage, v_module.passing_score_percentage, v_module.id, v_tp.id,
    v_module.property_id, v_module.department_id, 'active'
  )
