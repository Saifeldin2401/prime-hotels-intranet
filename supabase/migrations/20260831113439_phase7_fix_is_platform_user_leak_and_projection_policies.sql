...
-- is_platform_user(auth.uid()), which returns TRUE for legacy 'corporate_admin'/'regional_admin'
...
-- 1. is_platform_user: platform = super_admin only. corporate/regional_admin are tenant roles.
...
-- 2. Drop the redundant *_modify_tenant ALL policies — the canonical multitenant_*/org-scoped
--    policy sets already cover these tables and are stricter (capability-gated writes).
...
-- 3. Add the missing WITH CHECK to the course-hierarchy FOR ALL write policies (0 rows today,
--    but the bug class: rows could be written to a parent the author can't see).
