-- ============================================================================
-- MIGRATION: fix_create_task_atomic_nonexistent_column
-- create_task_atomic() -- the RPC every task creation in the app calls via
-- useCreateTask() -- referenced a column `assigned_to` that does not exist
-- on public.tasks (only assigned_to_id exists). Every task creation attempt
-- through this function has failed outright with a hard SQL error. tasks
-- has 0 rows, confirming this has never worked in this live database.
-- Discovered via a rolled-back functional test while wiring the new
-- Projects module's project_id support into this same function.
--
