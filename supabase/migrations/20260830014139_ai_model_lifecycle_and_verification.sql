...
COMMENT ON COLUMN public.ai_models.verified_at IS 'Set by the verifier the first time a real API probe of this model succeeded.';
COMMENT ON COLUMN public.ai_models.last_probe_ok IS 'Result of the most recent probe (any probe_type). NULL = never probed.';
COMMENT ON COLUMN public.ai_models.last_probe_at IS 'Timestamp of the most recent probe of this model.';
COMMENT ON COLUMN public.ai_models.capability_checked_at IS 'Last time a capability-specific probe (json mode / image) ran for this model.';

UPDATE public.ai_models
   SET verified_at = coalesce(verified_at, updated_at)
 WHERE availability = 'verified' AND verified_at IS NULL;
...
CREATE INDEX IF NOT EXISTS ai_model_probes_model_time_idx ON public.ai_model_probes (model_id, probed_at DESC);
CREATE INDEX IF NOT EXISTS ai_model_probes_time_idx ON public.ai_model_probes (probed_at DESC);
...
CREATE POLICY ai_model_probes_select ON public.ai_model_probes FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS ai_model_probes_write ON public.ai_model_probes;
CREATE POLICY ai_model_probes_write ON public.ai_model_probes FOR ALL TO authenticated
  ...
  SELECT jsonb_build_object(
    'models', coalesce((
      SELECT jsonb_agg(jsonb_build_object(
        'id', m.id, 'provider', m.provider, 'provider_model_id', m.provider_model_id,
        'display_name', m.display_name, 'modality', m.modality, 'cost_tier', m.cost_tier,
        'is_free', m.is_free, 'supports_json_object', m.supports_json_object,
        'image_generation', m.image_generation, 'availability', m.availability, 'enabled', m.enabled,
        'quality_score', m.quality_score, 'speed_score', m.speed_score,
        'verified_at', m.verified_at, 'last_probe_ok', m.last_probe_ok,
        'last_probe_at', m.last_probe_at, 'capability_checked_at', m.capability_checked_at
      ) ORDER BY m.provider, m.availability, m.id)
      FROM public.ai_models m
    ), '[]'::jsonb),
    'recent_probes', coalesce((
      SELECT jsonb_agg(jsonb_build_object(
        'id', p.id, 'model_id', p.model_id, 'probed_at', p.probed_at, 'ok', p.ok,
        'latency_ms', p.latency_ms, 'http_status', p.http_status, 'detail', p.detail, 'probe_type', p.probe_type
      ) ORDER BY p.probed_at DESC)
      FROM (SELECT * FROM public.ai_model_probes ORDER BY probed_at DESC LIMIT 100) p
    ), '[]'::jsonb),
    'generated_at', now()
  );
$function$;
...
CREATE OR REPLACE FUNCTION public.run_model_verification(
  p_provider text DEFAULT NULL, p_only_unverified boolean DEFAULT true
)
...
BEGIN
  ...
  SELECT net.http_post(
    ...
    body := jsonb_build_object('mode','all','provider',p_provider,'onlyUnverified',p_only_unverified,'source','run_model_verification_rpc'),
    timeout_milliseconds := 120000
  ) INTO v_req_id;

  RETURN jsonb_build_object('queued', true, 'net_request_id', v_req_id, 'provider', p_provider, 'only_unverified', p_only_unverified);
END;
$function$;
...
BEGIN
  ...
  EXCEPTION WHEN insufficient_privilege OR undefined_function THEN
    RAISE NOTICE 'Could not deactivate ai-model-verifier-nightly automatically; disable it manually.';
  END;
END $$;
