...
--    the tenant-isolation policy -> every authenticated user saw all 60 departments.
...
-- departments_tenant_isolation_admin (ALL, with WITH CHECK) already covers writes.
...
-- 3. unified_quiz_questions (link table quiz_id->question_id): the _manage ALL policy was a
--    legacy-role check. Isolate via the parent learning_quizzes org, add a SELECT policy.
...
USING (EXISTS (
  SELECT 1 FROM public.learning_quizzes lq
  WHERE lq.id = unified_quiz_questions.quiz_id
...
USING (EXISTS (
  SELECT 1 FROM public.learning_quizzes lq
  WHERE lq.id = unified_quiz_questions.quiz_id
    AND public.org_visible(lq.organization_id)
    AND public.is_tenant_content_editor(lq.organization_id)
))
WITH CHECK (EXISTS (
  SELECT 1 FROM public.learning_quizzes lq
  WHERE lq.id = unified_quiz_questions.quiz_id
