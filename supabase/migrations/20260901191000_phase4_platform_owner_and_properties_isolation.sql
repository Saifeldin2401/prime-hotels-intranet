...
--    the account running the takeover). Can be reassigned via user_roles.
...
--    (Full properties -> hotels retirement + FK repoint is a later phase; properties has
--     5 rows under one org and hotels mirrors it 1:1 by id.)
...
-- 3. brands_select_public was also USING(true) (cross-tenant brand list leak) -> drop it,
--    the org-scoped brands_tenant_isolation_select already covers legitimate reads.
