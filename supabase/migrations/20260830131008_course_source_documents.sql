CREATE TABLE IF NOT EXISTS public.course_source_documents (
  ...
  relationship        TEXT NOT NULL DEFAULT 'source' CHECK (relationship IN ('source', 'resource')),
...
CREATE INDEX IF NOT EXISTS course_source_documents_module_idx ON public.course_source_documents (training_module_id);
CREATE INDEX IF NOT EXISTS course_source_documents_document_idx ON public.course_source_documents (document_id);
...
CREATE POLICY course_source_documents_select ON public.course_source_documents
  ...
  USING (EXISTS (SELECT 1 FROM public.training_modules tm WHERE tm.id = training_module_id));
...
CREATE POLICY course_source_documents_write ON public.course_source_documents
  ...
  WITH CHECK (EXISTS (SELECT 1 FROM public.training_modules tm WHERE tm.id = training_module_id
      AND (tm.created_by = (SELECT auth.uid())
        OR EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = (SELECT auth.uid())
          AND (ur.role)::text = ANY (ARRAY['super_admin','corporate_admin','regional_admin','property_manager','property_hr'])))));
...
RETURNS TABLE (
  id uuid, document_id uuid, relationship text, is_primary boolean,
  original_filename text, file_type text, file_size bigint,
  attached_by uuid, attached_at timestamptz,
  doc_title text, doc_visibility text, doc_file_url text, doc_content_type text, caller_can_access boolean
)
LANGUAGE sql STABLE SECURITY INVOKER SET search_path TO ''
AS $function$
  SELECT csd.id, csd.document_id, csd.relationship, csd.is_primary,
    csd.original_filename, csd.file_type, csd.file_size, csd.attached_by, csd.attached_at,
    d.title, d.visibility::text, d.file_url, d.content_type, (d.id IS NOT NULL) AS caller_can_access
