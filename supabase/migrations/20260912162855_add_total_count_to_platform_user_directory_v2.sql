...
CREATE FUNCTION public.get_platform_user_directory(
...
RETURNS TABLE(
  ...
  is_platform_user boolean, platform_role text, primary_organization_id uuid,
  primary_organization_name text, membership_count bigint, memberships jsonb,
...
GRANT EXECUTE ON FUNCTION public.get_platform_user_directory(text, uuid, text, integer, integer) TO authenticated;
