...
-- visibility='all_properties'" branch had no tenant predicate, so any authenticated user
-- in any org saw every org's published org-wide documents in full (content + file_url).
-- Add a hard org gate to the whole visibility block.
