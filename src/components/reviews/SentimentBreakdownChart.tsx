        ...
        <CardContent>
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p className="text-xs mt-1">
              Run AI analysis on reviews to detect issue categories
            </p>
          </div>
        </CardContent>
      </Card>
    ...
    <Card className="border-none shadow-sm">
      <CardHeader className="pb-2">
        ...
        <CardDescription className="text-xs">
          Top complaint categories detected by AI — ranked by severity impact
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center gap-3 text-xs text-muted-foreground bg-muted/30 rounded-lg px-3 py-2">
          ...
          <span>
            <span className="font-semibold text-foreground">{withIssues}</span> of{' '}
            <span className="font-semibold text-foreground">{reviews.length}</span> reviews have
            AI-detected issues ({reviews.length > 0 ? Math.round((withIssues / reviews.length) * 100) : 0}%).{' '}
            <span className="text-green-600">{withoutIssues} reviews</span> had no flagged issues.
          </span>
        </div>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
                ...
                <th className="pb-1.5 text-right">High</th>
                <th className="pb-1.5 text-right">Medium</th>
                <th className="pb-1.5 text-center">Risk</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-muted/40">
              {categoryStats.map((cat) => {
                const riskScore = totalIssues > 0 ? Math.round((cat.criticalWeight / (totalIssues * 4)) * 100) : 0
                const riskLevel =
                  cat.critical > 0
                    ? { text: 'High Risk', cls: 'bg-red-100 text-red-700' }
                    : cat.high > 2
                    ? { text: 'Elevated', cls: 'bg-orange-100 text-orange-700' }
                    : cat.medium > 3
                    ? { text: 'Moderate', cls: 'bg-yellow-100 text-yellow-700' }
