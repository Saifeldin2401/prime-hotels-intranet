...
>(({ className, classNames, showOutsideDays = true, ...props }, _ref) => (
  <DayPicker
    ...
    classNames={{
      ...
      cell: "h-9 w-9 text-center text-sm p-0 relative [&:has([aria-selected].day-range-end)]:rounded-e-md [&:has([aria-selected].day-outside)]:bg-accent [&:has([aria-selected].day-outside)]:text-accent-foreground [&:has([aria-selected])]:bg-accent first:[&:has([aria-selected])]:rounded-s-md last:[&:has([aria-selected])]:rounded-e-md focus-within:relative focus-within:z-20",
      day: cn(
