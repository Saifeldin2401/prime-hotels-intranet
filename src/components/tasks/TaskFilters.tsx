...
export function TaskFilters({ filters, onChange }: TaskFiltersProps) {
  ...
  const normalizedPropertyId = isRealPropertyId(currentProperty?.id)
    ? currentProperty.id
    : undefined;
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                onClick={() => onChange({})}
                size="icon"
                aria-label={t("common:common.clear_filters")}
              >
                <X className="h-4 w-4" />
              </Button>
            </TooltipTrigger>
            <TooltipContent>{t("common:common.clear_filters")}</TooltipContent>
          </Tooltip>
        )}
      </div>
    </TooltipProvider>
