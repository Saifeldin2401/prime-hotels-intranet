...
/**
 * Hook specifically for form state that needs to persist
 */
export function usePersistentForm<T extends Record<string, unknown>>(
  formId: string,
  initialValues: T
) {
  const fields = useRef<Map<string, PersistentState<unknown>>>(new Map())
  
  const getField = useCallback(<K extends keyof T>(
    fieldName: K,
    defaultValue: T[K]
  ): PersistentState<T[K]> => {
    const key = `${formId}_${String(fieldName)}`
    
    // Return cached field if exists
    const cached = fields.current.get(key)
    if (cached) return cached as PersistentState<T[K]>
    
    // Create new persistent state for this field
    const field = usePersistentState(defaultValue, {
      key,
      backupKey: `${key}_backup`,
      debounceMs: 500,
    })
    
    fields.current.set(key, field as PersistentState<unknown>)
    return field
  }, [formId])
  ...
  const isAllHydrated = useCallback(() => {
    return Array.from(fields.current.values()).every(f => f.isHydrated)
  }, [])
