...
async function checkMediaAccess(assetId: string, userId: string): Promise<boolean> {
  try {
    // Check if user has any roles that grant media access
    ...
    // Non-admins: public assets, or assets of an organization they belong to
...
export function useMedia(options: UseMediaOptions = {}) {
  ...
  const fetchAssets = useCallback(async (filterOptions: MediaFilterOptions = {}) => {
    ...
    try {
      ...
      // Transform data to include the uploader name
      const transformedData = (data || []).map((item) => ({
        ...(item as unknown as MediaAsset),
        uploader_name: (item.uploader as { full_name?: string } | null)?.full_name || null,
      }));

      // 🤖 Also aggregate AI visual assets previously generated by Course Engine & Knowledge Base
      let courseAssets: MediaAsset[] = [];
      try {
        let cvaQuery = supabase
        ...
        if (filterOptions.searchQuery?.trim()) {
          cvaQuery = cvaQuery.ilike('title', `%${filterOptions.searchQuery.trim()}%`);
        }

        const { data: cvaData, error: cvaError } = await cvaQuery;
        if (!cvaError && cvaData) {
          courseAssets = cvaData.map((cva: any) => ({
            id: cva.id,
            title: cva.title || cva.caption || 'AI Course Visual',
            description: cva.prompt || cva.visual_concept || null,
            filename: `ai_course_${cva.id.slice(0, 8)}.png`,
            original_filename: cva.title ? `${cva.title}.png` : 'ai_generated.png',
            storage_path: cva.storage_path || cva.image_url || '',
            storage_bucket: cva.storage_bucket || 'content-media',
            public_url: cva.image_url || '',
            thumbnail_url: cva.image_url || null,
            media_type: 'image' as MediaType,
            category: (cva.educational_purpose === 'compliance' ? 'compliance' : 'training') as MediaCategory,
            file_size_bytes: 0,
            mime_type: cva.image_url?.includes('svg') ? 'image/svg+xml' : 'image/jpeg',
            width: null,
            height: null,
            ...
            tags: [
              'AI Generated',
              'Course Visual',
              cva.visual_style || 'educational_illustration',
              cva.model || 'ai-model',
            ].filter(Boolean),
            metadata: {
              is_ai_generated: true,
              model: cva.model,
              provider: cva.provider,
              prompt: cva.prompt,
              course_id: cva.course_id,
              lesson_id: cva.lesson_id,
            },
            usage_count: 1,
            ...
            virus_scan_score: 0,
            sha256_hash: null,
            scanned_at: cva.created_at || null,
            content_disposition: 'inline' as const,
            created_at: cva.created_at || new Date().toISOString(),
            updated_at: cva.updated_at || cva.created_at || new Date().toISOString(),
          }));
        }
      } catch (cvaErr) {
        console.warn('Deferred fetching course visual assets:', cvaErr);
      }

      // Merge avoiding duplicates
      const existingUrls = new Set(transformedData.map((a) => a.public_url).filter(Boolean));
      const existingIds = new Set(transformedData.map((a) => a.id));
      const uniqueCourseAssets = courseAssets.filter(
        (ca) => !existingIds.has(ca.id) && (!ca.public_url || !existingUrls.has(ca.public_url))
      );

      const allMergedAssets = [...transformedData, ...uniqueCourseAssets];
      setAssets(allMergedAssets);
    } catch (error) {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        return {
          ...asset,
          usages,
        } as unknown as MediaAssetWithUsage;
      }
      return null;
    } catch (error) {
  ...
  const uploadFile = useCallback(
    ...
    [fetchAssets, filters, organizationId, validateFileForUpload]
  );
  ...
  const getSecureDownloadUrl = useCallback(
    async (assetId: string): Promise<string | null> => {
      try {
        ...
        // SECURITY: Verify user has access to this file (owner or same organization)
  ...
  const updateAsset = useCallback(
    async (assetId: string, formData: MediaAssetFormData): Promise<boolean> => {
      try {
        ...
        if (error) {
          // Try updating course_visual_assets
          await supabase
            .from('course_visual_assets')
            .update({
              title: formData.title,
              caption: formData.description || null,
            })
            .eq('id', assetId);
        }
  ...
  const deleteAsset = useCallback(
    async (assetId: string): Promise<boolean> => {
      try {
        // Try deleting from media_assets
        const { data: asset } = await supabase
          .from('media_assets')
        ...
        if (asset) {
          ...
          // Try deleting from course_visual_assets
          await supabase.from('course_visual_assets').delete().eq('id', assetId);
        }
