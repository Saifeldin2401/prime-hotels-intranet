...
export default function Settings() {
    ...
    if (loading || settingsLoading) {
        return <div className="flex justify-center p-8"><Loader2 className="animate-spin text-ds-muted" /></div>
    }
    ...
    return (
        <div className={cn("mx-auto space-y-6", isBroadTab ? "max-w-6xl" : "max-w-4xl")}>
            ...
            <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
                <TabsList className="mb-6 h-auto w-full justify-start gap-1 overflow-x-auto rounded-none border-b border-ds-border bg-transparent p-0">
                    ...
                    <TabsTrigger value="security" className="min-h-[40px] rounded-none border-b-2 border-transparent px-3 text-sm text-ds-muted data-[state=active]:border-ds-ink data-[state=active]:bg-transparent data-[state=active]:text-ds-ink data-[state=active]:shadow-none">
                        {t('tabs.security')}
                    </TabsTrigger>
                    <TabsTrigger value="troubleshooting" className="min-h-[40px] rounded-none border-b-2 border-transparent px-3 text-sm text-ds-muted data-[state=active]:border-ds-ink data-[state=active]:bg-transparent data-[state=active]:text-ds-ink data-[state=active]:shadow-none">
                ...
                <TabsContent value="general">
                    <div className="grid gap-6">
                        ...
                        <Card className="overflow-hidden rounded-[6px] border border-ds-border bg-ds-surface shadow-none">
                            <CardHeader className="border-b border-ds-border">
                                <CardTitle className="flex items-center gap-2 text-base font-semibold text-ds-ink">
                                    <Globe className="h-4 w-4 text-ds-muted" aria-hidden="true" />
                                    {t('language.title')}
                                </CardTitle>
                                <CardDescription className="text-ds-muted">{t('language.description')}</CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-6 pt-6">
                                <div className="flex items-center justify-between">
                                    <div className="flex flex-col gap-1">
                                        <Label className="text-ds-ink font-medium">{t('language.label')}</Label>
                        ...
                        <Card className="overflow-hidden rounded-[6px] border border-ds-border bg-ds-surface shadow-none">
                            <CardHeader className="border-b border-ds-border">
                                <CardTitle className="flex items-center gap-2 text-base font-semibold text-ds-ink">
                                    <Accessibility className="h-4 w-4 text-ds-muted" aria-hidden="true" />
                                    {t('appearance.accessibility.title')}
                                </CardTitle>
                                <CardDescription className="text-ds-muted">{t_ext('tailor_the_interface_to_your_visual_and_', 'Tailor the interface to your visual and motor needs')}</CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-6 pt-6">
                                <div className="flex items-center justify-between">
                                    <div className="flex flex-col gap-1">
                                        <Label className="text-ds-ink font-medium">{t('appearance.accessibility.reduced_motion')}</Label>
                                        <p className="text-xs text-ds-muted">{t('appearance.accessibility.reduced_motion_desc')}</p>
                                    </div>
                                    <Switch
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex flex-col gap-1">
                                        <Label className="text-ds-ink font-medium">{t('appearance.accessibility.high_contrast')}</Label>
                                        <p className="text-xs text-ds-muted">{t('appearance.accessibility.high_contrast_desc')}</p>
                                    </div>
                                    <Switch
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex flex-col gap-1">
                                        <Label className="text-ds-ink font-medium">{t('appearance.accessibility.large_text')}</Label>
                                        <p className="text-xs text-ds-muted">{t('appearance.accessibility.large_text_desc')}</p>
                                    </div>
                                    <Switch
                                        checked={large_text}
                        ...
                        <Card className="overflow-hidden rounded-[6px] border border-ds-border bg-ds-surface shadow-none">
                            <CardHeader className="border-b border-ds-border">
                                <CardTitle className="flex items-center gap-2 text-base font-semibold text-ds-ink">
                                    <Keyboard className="h-4 w-4 text-ds-muted" aria-hidden="true" />
                                    {t('appearance.shortcuts.title')}
                                </CardTitle>
                                <CardDescription className="text-ds-muted">{t_ext('speed_up_your_workflow_with_power_user_t', 'Speed up your workflow with power-user tools')}</CardDescription>
                            </CardHeader>
                            <CardContent className="pt-6">
                                <div className="flex items-center justify-between">
                                    <div className="flex flex-col gap-1">
                                        <Label className="text-ds-ink font-medium">{t('appearance.shortcuts.enable')}</Label>
                                        <p className="text-xs text-ds-muted">{t('appearance.shortcuts.enable_desc')}</p>
                                    </div>
                                    <Switch
                        ...
                        <Card className="rounded-[6px] border border-ds-border bg-ds-surface shadow-none">
                            <CardHeader className="border-b border-ds-border">
                                <CardTitle className="text-base flex items-center gap-2">
                                    <Shield className="h-4 w-4 text-ds-muted" aria-hidden="true" />
                            ...
                            <CardContent className="pt-6 space-y-4">
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        className="w-full bg-ds-surface-subtle border border-border/50"
                                    />
                                    <p className="text-xs text-ds-muted">{t_ext('must_be_at_least_8_characters', 'Must be at least 8 characters')}</p>
                                </div>
                                <div className="space-y-2">
                                ...
                                <button
                                    ...
                                    className="w-full bg-ds-ink hover:bg-ds-ink/90 text-ds-on-ink font-semibold py-2 rounded-md transition-colors text-sm flex items-center justify-center gap-2"
                                >
                ...
                <TabsContent value="troubleshooting">
                    <div className="grid gap-6">
                        ...
                        <Card className="rounded-[6px] border border-ds-border bg-ds-surface shadow-none">
                            <CardHeader className="border-b border-ds-border">
                                <CardTitle className="text-base flex items-center gap-2">
                                    <Wrench className="h-4 w-4 text-ds-muted" aria-hidden="true" />
                            ...
                            <CardContent className="pt-6">
                                <div className="flex items-center justify-between gap-4">
                                    <div className="flex flex-col gap-1">
                                        <Label className="text-ds-ink font-medium">{t_ext('clear_cache_and_reload', 'Clear Cache & Reload')}</Label>
                                        <p className="text-xs text-ds-muted">{t_ext('seeing_a_blank_page_missing_data_or_a_st', "Seeing a blank page, missing data, or a stale version of the app? Clearing the cache and reloading can fix it.")}</p>
                                    </div>
                                    <button
                                        onClick={handleClearCache}
                                        disabled={clearingCache}
                                        className="shrink-0 flex items-center gap-2 text-sm text-ds-accent hover:text-ds-accent font-medium px-4 py-2 border border-ds-accent/30 rounded-md hover:bg-ds-accent/10 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
