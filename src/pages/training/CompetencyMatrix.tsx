    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="border border-border/50 bg-card/60 backdrop-blur-sm">
          ...
          <CardContent className="text-xs text-muted-foreground flex items-center gap-2">
            ...
            {isAr ? 'فجوة كفاءة أكبر من 1.0 مستوى عن المستهدف' : 'Competency gap >= 1.0 level below target'}
          </CardContent>
        </Card>
      </div>
      ...
      {filteredGaps.length === 0 ? (
        <Card className="p-10 text-center border-dashed">
          ...
          <h3 className="font-bold text-foreground">{isAr ? 'لا توجد بيانات كفاءات بعد' : 'No competency data yet'}</h3>
          <p className="text-sm text-muted-foreground mt-1">
            {isAr
              ? 'أضف كفاءات وقيّم الموظفين لعرض تحليل الفجوات'
              : 'Define competencies and evaluate staff to populate the gap analysis'}
          </p>
        </Card>
      ) : (
