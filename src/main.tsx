...
// Render once the active language is ready, so an Arabic session never flashes
// English (Arabic translations load as their own chunk). A failed language
// load still renders - English is always bundled as the fallback.
function renderApp() {
  try {
    root.render(
      <Wrapper>
...
void i18nReady.catch(() => undefined).finally(renderApp)
